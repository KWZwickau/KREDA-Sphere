...
class QueryBuilderTest extends \Doctrine\Tests\DbalTestCase
{
    ...
    public function testSelectWithAndOrWhereConditions()
    {
        ...
        $this->assertEquals( 'SELECT u.*, p.* FROM users u WHERE (((u.username = ?) AND (u.username = ?)) OR (u.name = ?)) AND (u.name = ?)',
            (string)$qb );
    }
    ...
    public function testSelectHavingAndHaving()
    {
        ...
        $this->assertEquals( 'SELECT u.*, p.* FROM users u GROUP BY u.id HAVING (u.name = ?) AND (u.username = ?)',
            (string)$qb );
    }
    ...
    public function testSelectHavingOrHaving()
    {
        ...
        $this->assertEquals( 'SELECT u.*, p.* FROM users u GROUP BY u.id HAVING (u.name = ?) OR (u.username = ?)',
            (string)$qb );
    }
    ...
    public function testSelectHavingAndOrHaving()
    {
        ...
        $this->assertEquals( 'SELECT u.*, p.* FROM users u GROUP BY u.id HAVING ((u.name = ?) OR (u.username = ?)) AND (u.username = ?)',
            (string)$qb );
    }
    ...
    public function testSelectFromMasterWithWhereOnJoinedTables()
    {
        ...
        $this->assertEquals( "SELECT COUNT(DISTINCT news.id) FROM newspages news INNER JOIN nodeversion nv ON nv.refId = news.id AND nv.refEntityname='Entity\\News' INNER JOIN nodetranslation nt ON nv.nodetranslation = nt.id INNER JOIN node n ON nt.node = n.id WHERE (nt.lang = ?) AND (n.deleted = 0)",
            $qb->getSQL() );
    }
}
