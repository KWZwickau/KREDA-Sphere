...
(function( register, global, undefined )
{
	...
	var Holder = {
		...
		run: function( userOptions )
		{
			...
			for (i = 0; i < bgnodes.length; i++) {
				var backgroundImage = global.getComputedStyle( bgnodes[i],
				...
				if (rawURL.indexOf( holderString ) === 0) {
					...
					if (rawURL.indexOf( holderString ) != -1) {
		...
		for (var fl = flags.length, j = 0; j < fl; j++) {
			...
			if (App.flags.dimensions.match( flag )) {
				...
				if (App.flags.fluid.match( flag )) {
					...
					if (App.flags.textmode.match( flag )) {
								...
								if (App.flags.font.match( flag )) {
									...
									if (App.flags.auto.match( flag )) {
										...
										if (App.flags.text.match( flag )) {
											...
											if (App.flags.random.match( flag )) {
	...
	function prepareDOMElement( mode, el, flags, _renderSettings )
	{
		...
		if (mode == 'image' || mode == 'fluid') {
			setAttr( el, {
				'alt': (theme.text ? (theme.text.length > 16 ? theme.text.substring( 0,
					16 ) + '…' : theme.text) + ' [' + dimensionsCaption + ']' : dimensionsCaption)
			} );
		}
		...
		if (mode == 'image') {
			...
			if (mode == 'background' && renderSettings.renderer != 'html') {
				render( mode, {
						dimensions: dimensions,
						theme: theme,
						flags: flags
	...
	function serializeSVG( svg, renderSettings )
	{
		...
		if (renderSettings.svgXMLStylesheet) {
			...
			for (var i = stylesheets.length - 1; i >= 0; i--) {
				var csspi = xml.createProcessingInstruction( 'xml-stylesheet',
	...
	function debounce( fn )
	{
		...
		if (App.vars.debounceTimer) {
			clearTimeout( App.vars.debounceTimer );
		}
	...
	function getNodeArray( val )
	{
		...
		if (typeof(val) == 'string') {
			...
			if (global.NodeList && val instanceof global.NodeList) {
				retval = val;
			} else
				if (global.Node && val instanceof global.Node) {
					retval = [val];
				} else
					if (global.HTMLCollection && val instanceof global.HTMLCollection) {
						retval = val;
					} else
						if (val === null) {
							retval = [];
