...
// MathJax.isPacked = true; // This line is uncommented by the packer.
        ...
        (function( BASENAME )
        {
            ...
            BASE.Object = OBJECT( {
                ...
                Init: function( args )
                {
                    ...
                    if (!(obj instanceof args.callee)) {
                        obj = new args.callee( PROTO )
                    }
                ...
                protoFunction: function( id, def )
                {
                    ...
                    if (typeof def === "function") {
                        def.SUPER = this.SUPER.prototype
                    }
                },
            ...
            BASE.Ajax = {
                ...
                fileName: function( url )
                {
                    ...
                    if (url.substr( 0, root.length ) === root) {
                        url = "[" + BASENAME + "]" + url.substr( root.length )
                    }
                    else {
                        for (var id in PATH) {
                            if (PATH.hasOwnProperty( id ) && PATH[id]) {
                ...
                removeHook: function( hook )
                {
                    if (this.loadHooks[hook.file]) {
                        ...
                        if (!this.loadHooks[hook.file].hooks.length) {
                ...
                Preloading: function()
                {
                    for (var i = 0, m = arguments.length; i < m; i++) {
                        ...
                        if (!this.loading[file]) {
                            this.loading[file] = {preloaded: true}
                        }
                    }
                },
                ...
                timer: {
                    ...
                    create: function( callback, node )
                    {
                        ...
                        if (node.nodeName === "STYLE" && node.styleSheet &&
                            ...
                            if (window.chrome && node.nodeName === "LINK") {
                                ...
                                if (isSafari2) {
                                    this.timer.start( this, [this.timer.checkSafari2, sheets++, callback],
                                        this.styleDelay );
                                } else {
                    ...
                    checkLength: function( check, node, callback )
                    {
                        ...
                        try {
                            if ((sheet.cssRules || sheet.rules || []).length > 0) {
                                isStyle = 1
                            }
                        } catch( err ) {
                            if (err.message.match( /protected variable|restricted URI/ )) {
                                isStyle = 1
                            }
                            else {
                                if (err.message.match( /Security error/ )) {
                ...
                if (contents) {
                    ...
                    for (var i = 0; i < contents.length; i++) {
                        if (contents[i] instanceof Array) {
                            ...
                            if (type === "script") { // IE throws an error if script is added as a text node
        ...
        MathJax.Localization = {
            ...
            _: function( id, phrase )
            {
                if (phrase instanceof Array) {
                    return this.processSnippet( id, phrase )
                }
            ...
            processString: function( string, args, domain )
            {
                ...
                for (i = 0, m = args.length; i < m; i++) {
                    if (domain && args[i] instanceof Array) {
                                ...
                                if (match) {
                                    if (match[1] === "plural") {
                                        ...
                                        if (typeof n === "undefined") {
                                            ...
                                            var plurals = match[3].replace( /(^|[^%])(%%)*%\|/g,
                                                "$1$2%\uEFEF" ).split( /\|/ ); // the parts (replacing %| with a special character)
                ...
                for (var i = 0, m = snippet.length; i < m; i++) {
                    if (snippet[i] instanceof Array) {
                        ...
                        if (typeof data[1] === "string") {        // [id,string,args]
                            ...
                            if (data[1] instanceof Array) {    // [domain,snippet]
                                result = result.concat( this.processSnippet.apply( this, data ) );
                            } else {
                                if (data.length >= 3) {            // ["tag",{properties},snippet]
            ...
            processMarkdown: function( phrase, args, domain )
            {
                ...
                for (var i = 1, m = parts.length; i < m; i += 8) {
                    if (parts[i + 1]) {        // stars (for bold/italic)
                        ...
                        if (parts[i + 3]) { //  backtics (for code)
                            //
                            //  Remove one leading or trailing space, and process substitutions
                            //  Make a <code> tag
                            //
                            data = this.processString( parts[i + 4].replace( /^\s/, "" ).replace( /\s$/, "" ), args,
                                domain );
                            if (!(data instanceof Array)) {
                                data = [data]
                            }
            ...
            setCSS: function( div )
            {
                ...
                if (locale) {
                    if (locale.fontFamily) {
                        div.style.fontFamily = locale.fontFamily
                    }
                    if (locale.fontDirection) {
                        div.style.direction = locale.fontDirection;
                        if (locale.fontDirection === "rtl") {
        ...
        MathJax.Message = {
            ...
            Set: function( text, n, clearDelay )
            {
                ...
                if (text instanceof Array) {
                    ...
                    try {
                        ...
                        if (!err.restart.called) {
                            ...
                            //
                            if (this.log[n].restarted == null) {
                                this.log[n].restarted = 0
                            }
                ...
                if (this.log[n].restarted) {
                    if (this.log[n].cleared) {
                        clearDelay = 0
                    }
                    if (--this.log[n].restarted === 0) {
                        delete this.log[n].cleared
                    }
                }
                //
            ...
            Clear: function( n, delay )
            {
                ...
                //
                if (this.log[n].prev != null) {
                    this.log[this.log[n].prev].next = this.log[n].next
                }
                if (this.log[n].next != null) {
            ...
            {
                var strings = [];
                for (var i = 1, m = this.log.length; i < m; i++) {
        ...
        MathJax.Hub = {
            ...
            {
                var config = this.config, id, parent;
                name = name.split( /\./ );
            ...
            isJax: function( element )
            {
                ...
                if (element && element.tagName != null && element.tagName.toLowerCase() === 'script') {
                    ...
                    if (element.type && this.inputJax[element.type.replace( / *;(.|\s)*/, "" )]) {
                        return -1
                    }
                }
                return 0;
                ...
                for (var i = 0, m = scripts.length; i < m; i++) {
                    ...
                    if (script.type && this.inputJax[script.type.replace( / *;(.|\n)*/, "" )]) {
                        if (script.MathJax) {
                            ...
                            if (script.MathJax.state !== STATE.PENDING) {
                        ...
                        if (script.MathJax.state !== STATE.PROCESSED) {
            ...
            processInput: function( state )
            {
                ...
                try {
                    ...
                    while (state.i < m) {
                        ...
                        if (prev && prev.className === "MathJax_Error") {
                        ...
                        if (!script.MathJax.elementJax || script.MathJax.state === STATE.UPDATE) {
                            ...
                            if (typeof jax === 'function') {                  // if a callback was returned
                                if (jax.called) {
                                    continue;
                                }                       //   go back and call Process() again
                            ...
                            if (script.MathJax.state === STATE.OUTPUT) {
                ...
                while (state.j < state.jaxIDs.length) {
                    ...
                    if (JAX[method]) {
                        try {
                            ...
                            if (!err.restart) {
                                MathJax.Message.Set( ["PrepError", "Error preparing %1 output (%2)", id, method], null,
                                    600 );
            ...
            processOutput: function( state )
            {
                ...
                try {
                    ...
                    while (state.i < m) {
                        ...
                        //
                        script = state.scripts[state.i];
                        if (!script || !script.MathJax || script.MathJax.error) {
                            state.i++;
                            continue
                        }
                        var jax = script.MathJax.elementJax;
            ...
            processError: function( err, state, type )
            {
                if (!err.restart) {
                    ...
                    this.formatError( state.scripts[state.i], err );
                    state.i++;
                }
            ...
            elementCallback: function( element, callback )
            {
                ...
                if (element == null) {
                    element = this.config.elements || []
                }
                if (this.isHTMLCollection( element )) {
...
//
        ...
        (function( BASENAME )
        {
            ...
            BASE.ElementJax = JAX.Subclass( {
                ...
                Clone: function( jax )
                {
                    ...
                    for (id in this) {
                        ...
                        if (typeof(jax[id]) === 'undefined' && id !== 'newID') {
                            delete this[id]
                        }
                    }
                    for (id in jax) {
            ...
            BASE.OutputJax.Error = {
                ...
                {
                    return BASE.Extension.MathEvents.Event.ContextMenu.apply( BASE.Extension.MathEvents.Event,
                        arguments )
                },
                Mousedown: function()
                {
                    return BASE.Extension.MathEvents.Event.AltContextMenu.apply( BASE.Extension.MathEvents.Event,
                        arguments )
                },
        ...
        (function( BASENAME )
        {
            ...
            for (var i = scripts.length - 1; i >= 0; i--) {
                if ((scripts[i].src || "").match( namePattern )) {
                    ...
                    CONFIG.root = scripts[i].src.replace( /(^|\/)[^\/]*(\?.*)?$/, '' )
                        ...
                        .replace( /^(https?:)\/\/[0-9a-f]+(-[0-9a-f]+)?.ssl.cf1.rackcdn.com\//,
                        "$1//cdn.mathjax.org/" );
            ...
            HUB.Browser = HUB.Insert( HUB.Insert( String( "Unknown" ), {version: "0.0"} ), BROWSERS );
            for (var browser in BROWSERS) {
                if (BROWSERS.hasOwnProperty( browser )) {
                    if (BROWSERS[browser] && browser.substr( 0, 2 ) === "is") {
                        browser = browser.slice( 2 );
                        if (browser === "Mac" || browser === "PC") {
                            continue;
                        }
                        HUB.Browser = HUB.Insert( String( browser ), BROWSERS );
            ...
            HUB.queue.Push(
                ...
                }, // indicates that MathJax is ready to process math
                ["Typeset", STARTUP],
                ["Hash", STARTUP],
                ["MenuZoom", STARTUP],
