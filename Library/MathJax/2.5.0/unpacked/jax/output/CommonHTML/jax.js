...
//      math.setTeXclass();
        ...
        {
            if (math.parentNode.className === "MathJax_CHTML_Display") {
        ...
        {
            return jax.root.CHTMLspanElement()
        },
    ...
    MathJax.Hub.Register.StartupHook( "mml Jax Ready", function()
    {
        ...
        MML.mbase.Augment( {
            ...
            CHTMLdefaultSpan: function( span, options )
            {
                ...
                if (this.isToken) {
                    this.CHTMLhandleToken( span );
                }
                for (var i = 0, m = this.data.length; i < m; i++) {
                    this.CHTMLaddChild( span, i, options );
                }
                return span;
            },
            CHTMLaddChild: function( span, i, options )
            {
                ...
                if (child) {
                    ...
                    if (!options.noBBox) {
                        ...
                        if (child.CHTML.h > this.CHTML.h) {
                            this.CHTML.h = child.CHTML.h;
                        }
                        if (child.CHTML.d > this.CHTML.d) {
                            this.CHTML.d = child.CHTML.d;
                        }
                        if (child.CHTML.t > this.CHTML.t) {
                            this.CHTML.t = child.CHTML.t;
                        }
                        if (child.CHTML.b > this.CHTML.b) {
                            this.CHTML.b = child.CHTML.b;
                        }
                    }
                } else {
                    if (options.forceChild) {
            ...
            CHTMLcreateSpan: function( span )
            {
                ...
                if (this.type === "mo" && this.data.join( "" ) === "\u222B") {
                    CHTML.lastIsInt = true
                }
                else {
                    if (this.type !== "mspace" || this.width !== "negativethinmathspace") {
                        CHTML.lastIsInt = false
                    }
                }
                //  ###
                if (!this.CHTMLspanID) {
                    this.CHTMLspanID = CHTML.GetID()
                }
            ...
            CHTMLhandleText: function( span, text )
            {
                ...
                for (var i = 0, m = text.length; i < m; i++) {
                    ...
                    if (CHTML.DELIMITERS[c]) {
                        w = CHTML.DELIMITERS[c].w || .4
                    }
                    // ### FIXME:  handle Greek
...
//      }},[text]);
            ...
            CHTMLcanStretch: function( direction, H, D )
            {
                if (this.isEmbellished()) {
                    var core = this.Core();
                    if (core && core !== this) {
                        return core.CHTMLcanStretch( direction, H, D )
                    }
                }
                return false;
            },
            ...
            CoreParent: function()
            {
                ...
                parent.CoreMO() === this && !parent.isa( MML.math )) {
            ...
            CoreText: function( parent )
            {
                ...
                parent.data.length === 1 && parent.data[0]) {
        ...
        MML.mo.Augment( {
            ...
            CHTMLstretchV: function( h, d )
            {
                ...
                span.appendChild( box );
                if (values.symmetric) {
                    span.style.verticalAlign = CHTML.Em( .25 * (1 - scale) );
                }
            }
        } );
        ...
        MML.mspace.Augment( {
            toCommonHTML: function( span )
            {
                ...
                if (w < 0) {
                    ...
                    if (!CHTML.lastIsInt) {
                        span.style.marginLeft = CHTML.Em( w );
                    }
                    w = 0;
                }
                ...
                if (d) {
                    span.style.verticalAlign = CHTML.Em( -d );
                }
                return span;
            }
        } );
            ...
            toCommonHTML: function( span )
            {
                ...
                if (values.width !== "") {
                    ...
                    if (dimen.pm) {
                        ...
                        span.style.width = CHTML.Em( dimen.len )
                    }
                }
                if (values.height !== "") {
                    dimen = this.CHTMLdimen( values.height, "h", 0 );
                    if (!dimen.pm) {
                        T += -this.CHTMLbboxFor( 0 ).h;
                    }
                    T += dimen.len;
                }
                if (values.depth !== "") {
                ...
                if (T) {
                    child.style.marginTop = CHTML.Em( T );
                }
                if (B) {
                    child.style.marginBottom = CHTML.Em( B );
                }
                if (L) {
                    child.style.marginLeft = CHTML.Em( L );
                }
                if (R) {
                    child.style.marginRight = CHTML.Em( R );
                }
                if (V) {
                    span.style.verticalAlign = CHTML.Em( V );
                }
                return span;
            },
        ...
        MML.msubsup.Augment( {
            toCommonHTML: function( span )
            {
                ...
                var bbox = this.CHTML;
                if (sup && sub) {
                    var box = HTML.Element( "span", {
                        className: "MJXc-script-box", style: {
        ...
        MML.mfrac.Augment( {
            toCommonHTML: function( span )
            {
                ...
                if (!values.displaystyle) {
                    if (this.data[0]) {
                        this.data[0].CHTMLhandleScriptlevel( span.firstChild );
                    }
                    if (this.data[1]) {
                        this.data[1].CHTMLhandleScriptlevel( span.lastChild );
                    }
                }
        ...
        MML.mroot.Augment( {
            toCommonHTML: function( span )
            {
                ...
                if (scale > 150) {
                    r *= .95 * Math.ceil( 150 / scale * 10 ) / 10
                }
                root.style.marginRight = CHTML.Em( r );
                root.style.verticalAlign = CHTML.Em( v );
                if (-r > rbox.w * .8) {
                    root.style.marginLeft = CHTML.Em( -r - rbox.w * .8 );
                } // ### depends on rbox.w
                span.appendChild( root );
...
//            RCALIGN = [];
                for (i = 0, m = CSPACE.length; i < m; i++) {
                    CSPACE[i] = CHTML.length2em( CSPACE[i] )
                }
                for (i = 0, m = RSPACE.length; i < m; i++) {
                    RSPACE[i] = CHTML.length2em( RSPACE[i] )
                }

                var table = HTML.Element( "span" );
                while (span.firstChild) {
                    table.appendChild( span.firstChild );
                }
                    ...
                    if (row) {
                        ...
                        for (j = 0, n = row.data.length; j < n - k; j++) {
                            ...
                            if (cell) {
                                ...
                                if (j) {
                                    rbox.w += cspace;
                                    cspan.style.paddingLeft = CHTML.Em( cspace )
                                }
                                if (i) {
                                    cspan.style.paddingTop = CHTML.Em( rspace );
                                }
