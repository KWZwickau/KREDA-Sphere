...
MathJax.Hub.Register.StartupHook( "SVG Jax Ready", function()
{
    ...
    MML.mfenced.Augment( {
        ...
        SVGmoveLine: function( start, end, svg, state, values )
        {
            ...
            if (i === j && start.length > 1) {
                ...
                //  If starting and ending in the same element move the subpiece to the new line
                //
                this.data[this.dataI[i]].SVGmoveSlice( start.slice( 1 ), end.slice( 1 ), svg, state, values,
                    "paddingLeft" );
            } else {
                //
                //  Otherwise, move the remainder of the initial item
    ...
    MML.msubsup.Augment( {
        ...
        SVGmoveLine: function( start, end, svg, state, values )
        {
            ...
            if (this.data[this.base]) {
                if (start.length > 1) {
                    this.data[this.base].SVGmoveSlice( start.slice( 1 ), end.slice( 1 ), svg, state, values,
                        "paddingLeft" );
                } else {
                    if (end.length <= 1) {
    ...
    MML.mmultiscripts.Augment( {
        ...
        SVGmoveLine: function( start, end, svg, state, values )
        {
            ...
            if (start.length < 1) {
                ...
                var presub = this.scriptBox[2], presup = this.scriptBox[3];
                dx = svg.w + data.dx;
                if (presup) {
            ...
            //
            if (end.length === 0) {
                var sub = this.scriptBox[0], sup = this.scriptBox[1];
                dx = svg.w + data.s;
                if (sup) {
    ...
    MML.mo.Augment( {
        ...
        SVGbetterBreak: function( info, state )
        {
            ...
            //
            var W = info.scanW, mo = info.embellished;
