...
(function( AJAX, HUB, HTML, SVG )
{
    ...
    SVG.Augment( {
        ...
        Zoom: function( jax, span, math, Mw, Mh )
        {
            ...
            var tw = jax.root.data[0].SVGdata.tw;
                ...
                if (Math.abs( svg.x ) < 1 && Math.abs( svg.y ) < 1) {
                    ...
                    if (nodeName === "g") {
                        if (!svg.element.firstChild) {
                            ...
                            svg.element.setAttribute( "transform",
                                "translate(" + Math.floor( svg.x ) + "," + Math.floor( svg.y ) + ")" )
                        }
                    } else {
                        if (nodeName === "line" || nodeName === "polygon" ||
        ...
        Stretch: function()
        {
            for (var i = 0, m = this.svg.length; i < m; i++) {
                ...
                if (mml) {
                    ...
                    mml.SVGdata.HW = this.sh;
                    mml.SVGdata.D = this.sd;
                }
                if (svg.ic) {
                    this.ic = svg.ic
                } else {
    ...
    BBOX.HLINE = BBOX.Subclass( {
        ...
        Init: function( w, t, dash, color, def )
        {
            ...
            if (dash === "dashed") {
                var n = Math.floor( Math.max( 0, w - t ) / (6 * t) ), m = Math.floor( Math.max( 0,
                    w - t ) / (2 * n + 1) );
    ...
    BBOX.VLINE = BBOX.Subclass( {
        ...
        Init: function( h, t, dash, color, def )
        {
            ...
            if (dash === "dashed") {
                var n = Math.floor( Math.max( 0, h - t ) / (6 * t) ), m = Math.floor( Math.max( 0,
                    h - t ) / (2 * n + 1) );
                def["stroke-dasharray"] = m + " " + m;
            }
            if (dash === "dotted") {
    ...
    HUB.Register.StartupHook( "mml Jax Ready", function()
    {
        ...
        MML.mbase.Augment( {
            ...
            SVGsaveData: function( svg )
            {
                ...
                if (svg.y) {
                    this.SVGdata.h += svg.y;
                    this.SVGdata.d -= svg.y
                }
                if (svg.X != null) {
                    this.SVGdata.X = svg.X
                }
                if (svg.tw != null) {
                    this.SVGdata.tw = svg.tw
                }
                if (svg.skew) {
                    this.SVGdata.skew = svg.skew
                }
                if (svg.ic) {
                    this.SVGdata.ic = svg.ic
                }
                if (this["class"]) {
                    svg.removeable = false;
                    SVG.Element( svg.element, {"class": this["class"]} )
                }
                ...
                if (this.id) {
                    svg.removeable = false;
                    SVG.Element( svg.element, {"id": this.id} )
                }
                if (this.href) {
                    ...
                    SVG.addElement( a, "rect", {
                        width: svg.w, height: svg.h + svg.d, y: -svg.d,
            ...
            SVGhandleColor: function( svg )
            {
                ...
                if (values.background !== MML.COLOR.TRANSPARENT) {
                    ...
                    svg.Add( BBOX.RECT( svg.h, svg.d, svg.w, {fill: values.background, stroke: "none"} ), 0, 0, false,
                        true )
                }
                //
                //  Add borders
                    ...
                    for (id in sides) {
                        if (sides.hasOwnProperty( id )) {
                            if (borders[id]) {
                                ...
                                svg.Add( box( side[1], borders[id], borders[id + "Style"], borders[id + "Color"] ),
                                    side[2], side[3] );
                            }
                        }
                    }
        ...
        MML.msubsup.Augment( {
            toSVG: function( HW, D )
            {
                ...
                if (this.data[this.base] &&
                    ...
                    if (this.data[this.base].data.join( "" ).length === 1 && base.scale === 1 && !base.stretched && !this.data[this.base].Get( "largeop" )) {
                        u = v = 0
                    }
                }
        ...
        MML.math.Augment( {
            ...
            toSVG: function( span, div )
            {
                ...
                if (this.data[0]) {
                    ...
                    svg.element.setAttribute( "viewBox",
                        SVG.Fixed( -l, 1 ) + " " + SVG.Fixed( -svg.H - SVG.em, 1 ) + " " +
...
//        span.setAttribute("tabindex",0);  // causes focus outline, so disable for now
                    ...
                    if (!this.isMultiline && this.Get( "display" ) === "block" && !svg.hasIndent) {
                        var values = this.getValues( "indentalignfirst", "indentshiftfirst", "indentalign",
                            "indentshift" );
