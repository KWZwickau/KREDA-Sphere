    ...
    MML.mmultiscripts.Augment( {
        toHTML: function( span, HW, D )
        {
            ...
            if (this.data[this.base] &&
                ...
                if (this.data[this.base].data.join( "" ).length === 1 && base.bbox.scale === 1 && !this.data[this.base].Get( "largeop" )) {
                    u = v = 0
                }
            }
            ...
            var dx = 0;
            if (presub) {
                dx = presub.bbox.w + delta
            } else {
                if (presup) {
                    dx = presup.bbox.w - delta
                }
            }
            if (dx < 0) {
                dx = 0
            ...
            if (!sup && !presup) {
                ...
                if (!sub && !presub) {
                    ...
                    if (presub) {
                        h = Math.max( h, presub.bbox.h )
                    }
                    if (presup) {
                        d = Math.max( d, presup.bbox.d )
                    }
                    if ((u - d) - (h - v) < 3 * t) {
                        v = 3 * t - u + d + h;
                        q = (4 / 5) * x_height - (u - d);
                    ...
                    u = Math.max( u, min.superscriptshift );
                    v = Math.max( v, min.subscriptshift );
                    if (sup) {
                        HTMLCSS.placeBox( sup, dx + base.bbox.w + s, u )
                    }
                    if (presup) {
                        HTMLCSS.placeBox( presup, dx + delta - presup.bbox.w, u )
                    }
                    if (sub) {
                        HTMLCSS.placeBox( sub, dx + base.bbox.w + s - delta, -v )
                    }
                    if (presub) {
                        HTMLCSS.placeBox( presub, dx - presub.bbox.w, -v )
                    }
                }
            }
                    ...
                    for (var j = k; j < k + 2; j++) {
                        if (this.data[i] && this.data[i].type !== "none") {
                            if (!BOX[j]) {
                                ...
                                BOX[j].bbox = this.HTMLemptyBBox( {} );
                                if (W) {
                                    HTMLCSS.createBlank( BOX[j], W );
                                    BOX[j].bbox.w = BOX[j].bbox.rw = W
                                }
                            }
                            this.data[i].toHTML( BOX[j] );
                            this.HTMLcombineBBoxes( this.data[i], BOX[j].bbox );
                        }
                        i++;
                    }
                    sub = BOX[k];
