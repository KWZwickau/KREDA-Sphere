...
MathJax.Hub.Register.StartupHook( "HTML-CSS Jax Ready", function()
{
    ...
    MML.mfenced.Augment( {
        ...
        HTMLmoveLine: function( start, end, span, state, values )
        {
            ...
            if (i === j && start.length > 1) {
                ...
                //  If starting and ending in the same element move the subpiece to the new line
                //
                this.data[this.dataI[i]].HTMLmoveSlice( start.slice( 1 ), end.slice( 1 ), span, state, values,
                    "paddingLeft" );
            } else {
                //
                //  Otherwise, move the remainder of the initial item
    ...
    MML.msubsup.Augment( {
        ...
        HTMLmoveLine: function( start, end, span, state, values )
        {
            ...
            if (this.data[this.base]) {
                if (start.length > 1) {
                    this.data[this.base].HTMLmoveSlice( start.slice( 1 ), end.slice( 1 ), span, state, values,
                        "paddingLeft" );
                } else {
                    if (end.length <= 1) {
