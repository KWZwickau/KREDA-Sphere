    ...
    MML.mtable.Augment( {
        toHTML: function( span )
        {
                    ...
                    if (cell.isEmbellished()) {
                        ...
                        if (min) {
                            ...
                            if (mo.HTMLcanStretch( "Vertical" )) {
                                ...
                                if (mo.HTMLcanStretch( "Horizontal" )) {
            ...
            for (i = 0, m = CSPACE.length; i < m; i++) {
                CSPACE[i] = HTMLCSS.length2em( CSPACE[i], mu )
            }
            for (i = 0, m = RSPACE.length; i < m; i++) {
                RSPACE[i] = HTMLCSS.length2em( RSPACE[i], mu )
            }
            ...
            if (values.equalcolumns && values.width !== "auto") {
                    ...
                    if (CWIDTH[i] === "auto") {
                        ...
                        if (CWIDTH[i] === "fit") {
                            ...
                            if (CWIDTH[i].match( /%$/ )) {
                                ...
                                p++;
                                Wp += W[i];
                                WP += HTMLCSS.length2em( CWIDTH[i], mu, 1 )
                            }
                            else {
                                W[i] = HTMLCSS.length2em( CWIDTH[i], mu );
                                Wt += W[i]
                            }
                        }
                    }
                ...
                if (hasRelativeWidth) {
                    ...
                    WW = 0;
                    for (i = 0, m = Math.min( J, CSPACE.length ); i < m; i++) {
                        WW += CSPACE[i]
                    }
                    if (WP > .98) {
                        Wf = .98 / WP;
                    ...
                    for (i = 0, m = P.length; i < m; i++) {
                        W[P[i]] = HTMLCSS.length2em( CWIDTH[P[i]], mu, WW * Wf );
                        Wt += W[P[i]];
                    }
                ...
                for (i = 0, m = A.length; i < m; i++) {
                    if (A[i][j]) {
                        ...
                        if (cell.HTMLcanStretch( "Horizontal" )) {
                            ...
                            if (cell.HTMLcanStretch( "Vertical" )) {
                                mo = cell.CoreMO();
                                ...
                                A[i][j].HH = null;
                                if (A[i][j].bbox.h > H[i]) {
                                    A[i][j].bbox.H = A[i][j].bbox.h;
                                    A[i][j].bbox.h = H[i]
                                }
                                if (A[i][j].bbox.d > D[i]) {
                                    A[i][j].bbox.D = A[i][j].bbox.d;
                                    A[i][j].bbox.d = D[i]
                                }
                        ...
                        dy = ({
                            ...
                            baseline: 0, axis: 0
                        })[align] || 0; // FIXME:  handle axis better?
            ...
            if (hasRelativeWidth) {
                ...
                box.style.right = HTMLCSS.Em( WW + 2 * fx );
                box.style.display = "inline-block";
                box.style.height = "0px";
                if (HTMLCSS.msieRelativeWidthBug) {
                    box = HTMLCSS.createBox( box );
                ...
                for (j = 0; j <= J; j++) {
                    ...
                    var Cj = HTMLCSS.createBox( box );
                    HTMLCSS.addBox( Cj, C[j] );
                    C[j] = Cj;
                    var CjStyle = Cj.style;
                    CjStyle.display = "inline-block";
                    CjStyle.left = xp + "%";
                    //
                    ...
                    if (CWIDTH[j].match( /%$/ )) {
                        ...
                        if (f === 0) {
                            CjStyle.width = (wp + pp) + "%";
                            xp += wp + pp;
                            Cj = HTMLCSS.createBox( Cj );
                            HTMLCSS.addBox( Cj, C[j].firstChild );
                            Cj.style.left = 0;
                            Cj.style.right = HTMLCSS.Em( wm );
                            xf -= wm;
                        } else {
                            CjStyle.width = pp + "%";
                            xp += pp;
                        ...
                        if (CWIDTH[j] === "fit" || f === 0) {
                            ...
                            HTMLCSS.addBox( Cj, C[j].firstChild );
                            Cj.style.left = 0;
                            Cj.style.right = HTMLCSS.Em( wm - W[j] );
                            xf += W[j] - wm;
                            xp += wp;
                        } else {
                            CjStyle.width = HTMLCSS.Em( W[j] );
                            xf += W[j];
                        }
                    }
                    if (HTMLCSS.msieRelativeWidthBug) {
                    ...
                    if (CLINES[j] !== "none" && j < J && j !== LABEL) {
                        line = HTMLCSS.createBox( box );
                        line.style.left = xp + "%";
                        line = HTMLCSS.createRule( line, fH, 0, 1.25 / HTMLCSS.em );
                        line.style.position = "absolute";
                        line.bbox = {h: fH, d: 0, w: 0, rw: 1.25 / HTMLCSS.em, lw: 0};
                        line.parentNode.bbox = stack.bbox; // make sure stack size is updated
                        HTMLCSS.placeBox( line, xf - CSPACE[j] / 2, fY, true );
                        line.style.borderStyle = CLINES[j];
                    }
                }
            } else {
                //
                ...
                for (j = 0; j <= J; j++) {
                    if (!C[j].bbox.width) {
                        HTMLCSS.setStackWidth( C[j], W[j] )
                    }
                    if (CWIDTH[j] !== "auto" && CWIDTH[j] !== "fit") {
                        C[j].bbox.width = W[j];
                        C[j].bbox.isFixed = true
                    }
                    ...
                    //
                    if (CLINES[j] !== "none" && j < J && j !== LABEL) {
                        line = HTMLCSS.createRule( stack, fH, 0, 1.25 / HTMLCSS.em );
                        HTMLCSS.addBox( stack, line );
                        line.bbox = {h: fH, d: 0, w: 0, rw: 1.25 / HTMLCSS.em, lw: 0};
                        HTMLCSS.placeBox( line, x - CSPACE[j] / 2, fY, true );
                        line.style.borderStyle = CLINES[j];
                    }
                }
            }
            ...
            if (values.frame !== "none") {
                ...
                HTMLCSS.placeBox( frame, 0, fY, true );
                if (hasRelativeWidth) {
            ...
            for (i = 0, m = A.length - 1; i < m; i++) {
                ...
                if (RLINES[i] !== "none") {
                    line = HTMLCSS.createRule( stack, 1.25 / HTMLCSS.em, 0, fW );
                    HTMLCSS.addBox( stack, line );
                    ...
                    if (RLINES[i] === "dashed" || hasRelativeWidth) {
                        line.style.borderTop = line.style.height + " " + RLINES[i];
                        line.style.height = 0;
                        line.style.width = line.style.borderLeftWidth;
            ...
            if (C[LABEL]) {
                ...
                if (indent.indentalignfirst !== MML.INDENTALIGN.INDENTALIGN) {
                    indent.indentalign = indent.indentalignfirst
                }
                if (indent.indentalign === MML.INDENTALIGN.AUTO) {
                    indent.indentalign = this.displayAlign
                }
                if (indent.indentshiftfirst !== MML.INDENTSHIFT.INDENTSHIFT) {
                    indent.indentshift = indent.indentshiftfirst
                }
                if (indent.indentshift === "auto") {
                ...
                HTMLCSS.addBox( eqn, stack );
                HTMLCSS.alignBox( stack, indent.indentalign, 0, shift );

                C[LABEL].parentNode.parentNode.removeChild( C[LABEL].parentNode );
                HTMLCSS.addBox( eqn, C[LABEL] );
                HTMLCSS.alignBox( C[LABEL], CALIGN[LABEL], 0 );
                if (HTMLCSS.msieRelativeWidthBug) {
                    stack.style.top = C[LABEL].style.top = ""
                }
                if (hasRelativeWidth) {
                    stack.style.width = values.width;
                    span.bbox.width = "100%"
                }
                C[LABEL].style.marginRight = C[LABEL].style.marginLeft = HTMLCSS.Em( labelshift );
                if (indent.indentalign === MML.INDENTALIGN.CENTER) {
                    mw += 4 * labelshift + 2 * C[LABEL].bbox.w
                }
                else {
                    if (indent.indentalign !== CALIGN[LABEL]) {
                        mw += 2 * labelshift + C[LABEL].bbox.w
                    }
                }
                mw = Math.max( 0, mw + shift );
                span.bbox.tw = mw + 2 * labelshift;
        ...
        {
            span.bbox.keepPadding = true;
            span.bbox.exact = true;
    ...
    MML.mtd.Augment( {
        toHTML: function( span, HW, D )
        {
            ...
            if (this.data[0]) {
                ...
                if (D != null) {
                    box = this.data[0].HTMLstretchV( span, HW, D )
                }
                else {
                    if (HW != null) {
                        box = this.data[0].HTMLstretchH( span, HW )
                    }
                }
                span.bbox = box.bbox;
            }
