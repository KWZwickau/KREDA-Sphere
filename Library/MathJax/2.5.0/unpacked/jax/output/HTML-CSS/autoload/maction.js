...
MathJax.Hub.Register.StartupHook( "HTML-CSS Jax Ready", function()
{
    ...
    MML.maction.Augment( {
        ...
        toHTML: function( span, HW, D )
        {
            ...
            if (D != null) {
                HTMLCSS.Remeasured( selected.HTMLstretchV( span, HW, D ), span )
            }
            else {
                if (HW != null) {
                    HTMLCSS.Remeasured( selected.HTMLstretchH( span, HW ), span )
                } else {
                    HTMLCSS.Measured( box, span )
                }
            }
        ...
        HTMLhandleHitBox: function( span, postfix )
        {
            ...
            if (this.HTMLaction[type]) {
                this.HTMLaction[type].call( this, span, frame, this.Get( "selection" ) )
            }
        },
        ...
        HTMLtooltipPost: function( x, y )
        {
            ...
            var scale = Math.floor( Math.max( HTMLCSS.config.minScaleAdjust / 100,
                (ex / HTMLCSS.TeX.x_height) / em ) * HTMLCSS.config.scale );
            ...
            try {
                HTMLCSS.Measured( this.data[1].toHTML( box ), box )
            } catch( err ) {
                if (!err.restart) {
                    throw err
                }
