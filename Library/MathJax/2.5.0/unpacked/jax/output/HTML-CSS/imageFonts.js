    ...
    HUB.Register.LoadHook( HTMLCSS.fontDir + "/fontdata.js", function()
    {
        ...
        HTMLCSS.Augment( {
            ...
            handleImg: function( span, font, c, n, text )
            {
                ...
                var orig = c[5].orig;
                if (!orig) {
                    orig = c[5].orig = [c[0], c[1], c[2], c[3], c[4]]
                }
                var bscale = this.imgZoom;
                if (!span.scale) {
                    span.scale = 1
                }
                ...
                var chr = n.toString( 16 ).toUpperCase();
                while (chr.length < 4) {
                    chr = "0" + chr
                }
                ...
                var style = {
                    width: Math.floor( img[0] / bscale + .5 ) + "px",
                    height: Math.floor( img[1] / bscale + .5 ) + "px"
                };
                if (img[2]) {
                    style.verticalAlign = Math.floor( -img[2] / bscale + .5 ) + "px"
                }
                if (c[3] < 0) {
                    style.marginLeft = this.Em( c[3] / 1000 )
                }
                if (c[4] != c[2]) {
                    style.marginRight = this.Em( (c[2] - c[4]) / 1000 )
                }
                if (this.msieIE6) {
                ...
                for (var font in def) {
                    if (def.hasOwnProperty( font )) {
                        ...
                        if (FONT) {
                            font = def[font];
                            for (var n in font) {
                                if (font.hasOwnProperty( n ) && FONT[n]) {
                                    FONT[n][5] = {img: font[n]}
                                }
                            }
                        }
            ...
            initImg: function( span )
            {
                ...
                for (var i = 0, m = this.imgEmWidth.length - 1; i < m; i++) {
                    if (this.em <= this.imgEmWidth[i]) {
                        break
                    }
                }
                if (i && this.imgEmWidth[i] - this.em > this.em - this.imgEmWidth[i - 1]) {
                    i--
                }
            ...
            imgIndex: function( scale )
            {
                ...
                if (this.imgSizeForScale[this.em][scale]) {
                    return this.imgSizeForScale[this.em][scale]
                }
                var em = this.em * scale;
                for (var i = 0, m = this.imgEmWidth.length - 1; i < m; i++) {
                    if (em <= this.imgEmWidth[i]) {
                        break
                    }
                }
                if (i && this.imgEmWidth[i] - em > em - this.imgEmWidth[i - 1]) {
                    i--
                }
