...
(function( AJAX, HUB, HTMLCSS )
{
    ...
    var FONTTEST = MathJax.Object.Subclass( {
        ...
        Init: function()
        {
            ...
            this.div = MathJax.HTML.addElement( document.body, "div", {
                    ...
                    [
                        ...
                        style: {
                            position: "absolute", visibility: "hidden", top: 0, left: 0, width: "auto",
        ...
        findFont: function( fonts, pref )
        {
            ...
            if (pref && this.testCollection( pref )) {
                ...
                for (var i = 0, m = fonts.length; i < m; i++) {
                    ...
                    if (this.testCollection( fonts[i] )) {
                        found = fonts[i];
                        break
                    }
                }
            }
            if (found === "STIX" && this.testFont( this.FedoraSTIXcheck )) {
                found = null
            }
            return found;
        },
                ...
                if (found === null && (this.div.offsetWidth != W[3] || this.div.offsetHeight != W[4])) {
                    if (!font.noStyleChar && HTMLCSS.FONTDATA && HTMLCSS.FONTDATA.hasStyleChar) {
                        for (var i = 0, m = this.testSize.length; i < m; i++) {
                            if (this.testStyleChar( font, this.testSize[i] )) {
                                found = true;
                                m = 0
                            }
                        }
        ...
        testStyleChar: function( font, size )
        {
            ...
            var extra = "", dw = 0;
            var SIZE = this.div.style.fontSize;
            ...
            if (HTMLCSS.safariTextNodeBug) {
                this.div.innerHTML = this.compChar + extra
            }
            else {
                this.text.nodeValue = this.compChar + extra
            }
            var W = this.div.offsetWidth - dw;
            if (HTMLCSS.safariTextNodeBug) {
                this.div.innerHTML = this.styleChar + extra
            }
            else {
                this.text.nodeValue = this.styleChar + extra
            }
            var N = Math.floor( (this.div.offsetWidth - dw) / W + .5 );
            if (N === n) {
                if (HTMLCSS.safariTextNodeBug) {
                    this.div.innerHTML = this.versionChar + extra
                }
                else {
                    this.text.nodeValue = this.versionChar + extra
                }
        ...
        getComparisonWidths: function( string, noStyleChar )
        {
            ...
            for (var i = 1, m = this.comparisonFont.length; i < m; i++) {
                ...
                if (this.div.offsetWidth != W) {
                    return [W, this.div.offsetWidth, i, sW, sH]
                }
            }
            return null;
        },
        ...
        {
            MESSAGE( ["CantLoadWebFont", "Can't load web font %1", HTMLCSS.fontInUse + "/" + font.directory], null,
                2000 );
            HUB.Startup.signal.Post( [
                "HTML-CSS Jax - web font error",
                HTMLCSS.fontInUse + "/" + font.directory,
                font
            ] );
        },
        ...
        fontFace: function( name )
        {
            ...
            if (HTMLCSS.msieFontCSSBug && !FONT.family.match( /-Web$/ )) {
                FONT.family += "-Web"
            }
    ...
    HTMLCSS.Augment( {
        ...
        Config: function()
        {
            ...
            if (this.adjustAvailableFonts) {
                this.adjustAvailableFonts( config.availableFonts )
            }
            if (settings.scale) {
            ...
            if (font && font !== "Auto" && this.fontName[font]) {
                ...
                if (config.preferredFont) {
                    config.availableFonts[0] = config.preferredFont
                }
            }
            if (config.fonts) {
                config.availableFonts = config.fonts;
                config.preferredFont = config.webFont = config.fonts[0];
                if (config.webFont === "STIX") {
                    config.webFont += "-Web"
                }
            }
            font = this.Font.findFont( config.availableFonts, config.preferredFont );
            if (!font && this.allowWebFonts) {
                font = config.webFont;
                if (font) {
                    this.webFonts = true
                }
            }
            if (!font && this.config.imageFont) {
                font = config.imageFont;
                this.imgFonts = true
            }
            if (font) {
                this.fontInUse = font;
                this.fontDir += "/" + font;
                this.webfontDir += "/" + font;
        ...
        Startup: function()
        {
            ...
            this.hiddenDiv = this.Element( "div", {
                style: {
                    visibility: "hidden", overflow: "hidden", position: "absolute", top: 0,
        ...
        removeSTIXfonts: function( fonts )
        {
            ...
            if (this.config.preferredFont === "STIX") {
                this.config.preferredFont = fonts[0]
            }
        },

        PreloadWebFonts: function()
        {
            if (!HTMLCSS.allowWebFonts || !HTMLCSS.config.preloadWebFonts) {
                return;
            }
        ...
        InitializeHTML: function()
        {
            ...
            {
                if (check.time( ready )) {
                    HUB.signal.Post( ["HTML-CSS Jax - no default em size"] );
                    return
                }
        ...
        Translate: function( script, state )
        {
            ...
            if (this.hideProcessedMath) {
                ...
                //
                state.HTMLCSSeqn += (state.i - state.HTMLCSSi);
                state.HTMLCSSi = state.i;
        ...
        {
            if (math.parentNode.className === "MathJax_Display") {
        ...
        {
            return jax.root.HTMLspanElement()
        },
        ...
        Zoom: function( jax, span, math, Mw, Mh )
        {
            ...
            if (width) {
                ...
                if (child) {
                    ...
                    if (cwidth > Mw) {
                        span.style.width = (cwidth + 100) + "px"
                    }
                }
            }
            //
            ...
            if (bbox.lw < 0) {
                child.paddingLeft = HTMLCSS.Em( -bbox.lw )
            }
            if (bbox.rw > bbox.w) {
                child.marginRight = HTMLCSS.Em( bbox.rw - bbox.w )
            }
            //
        ...
        initFont: function( name )
        {
            ...
            if (HTMLCSS.FontFaceBug) {
                ...
                if (HTMLCSS.msieFontCSSBug) {
                    FONTS[name].family += "-Web"
                }
            }
        ...
        getHD: function( span )
        {
            if (span.bbox && this.config.noReflows) {
                return {h: span.bbox.h, d: span.bbox.d}
            }
        ...
        getW: function( span )
        {
            ...
            if (span.bbox && span.bbox.exactW) {
                return w
            }
            if ((span.bbox && w >= 0 && !this.initialSkipBug && !this.msieItalicWidthBug) ||
                this.negativeBBoxes || !span.firstChild) {
                W = span.offsetWidth;
                H = span.parentNode.offsetHeight;
            } else {
                if (span.bbox && w < 0 && this.msieNegativeBBoxBug) {
                    ...
                    span.insertBefore( start, span.firstChild );
        ...
        MeasureSpans: function( SPANS )
        {
            ...
            for (i = 0, m = SPANS.length; i < m; i++) {
                ...
                if (this.negativeBBoxes || !span.firstChild || (bbox.w >= 0 && !this.initialSkipBug) ||
                    ...
                    if (this.initialSkipBug) {
                        start = this.startMarker.cloneNode( true );
                        end = this.endMarker.cloneNode( true );
                        span.insertBefore( start, span.firstChild );
                        span.appendChild( end );
                        spans.push( [span, start, end, span.style.position] );
                        span.style.position = "absolute";
                    } else {
                        end = this.endMarker.cloneNode( true );
                        span.appendChild( end );
                        spans.push( [span, null, end] );
                    }
                }
            }
            //
            ...
            for (i = 0, m = spans.length; i < m; i++) {
                ...
                if ((bbox.w >= 0 && !this.initialSkipBug) || this.negativeBBoxes || !span.firstChild) {
                    W = span.offsetWidth;
                    parent.HH = parent.offsetHeight / this.em;
                } else {
                    if (bbox.w < 0 && this.msieNegativeBBoxBug) {
            ...
            for (i = 0, m = spans.length; i < m; i++) {
                span = spans[i];
                if (span[1]) {
                    span[1].parentNode.removeChild( span[1] ), span[0].style.position = span[3]
                }
                if (span[2]) {
                    span[2].parentNode.removeChild( span[2] )
                }
            }
        },
        ...
        {
            if (borders) {
                for (var id in borders.css) {
                    if (borders.css.hasOwnProperty( id )) {
        ...
        createStrut: function( span, h, before )
        {
            ...
            if (before) {
                span.insertBefore( strut, span.firstChild )
            } else {
        ...
        createBlank: function( span, w, before )
        {
            ...
            if (w < 0) {
                blank.style.marginRight = blank.style.width;
                blank.style.width = 0
            }
            if (before) {
                span.insertBefore( blank, span.firstChild )
            } else {
        ...
        createShift: function( span, w, before )
        {
            ...
            if (before) {
                span.insertBefore( space, span.firstChild )
            } else {
        ...
        createSpace: function( span, h, d, w, color, isSpace )
        {
            if (h < -d) {
                d = -h
            } // make sure h is above d
            var H = this.Em( h + d ), D = this.Em( -d );
            if (this.msieInlineBlockAlignBug) {
                D = this.Em( HTMLCSS.getHD( span.parentNode ).d - d )
            }
        ...
        createRule: function( span, h, d, w, color )
        {
            ...
            var rule = this.addElement( span, "span", {
                style: {
                    borderLeft: color, display: "inline-block", overflow: "hidden",
            ...
            if (w > 0 && !this.noReflows && rule.offsetWidth == 0) {
                rule.style.width = this.Em( w )
            }
            if (span.isBox || span.className == "mspace") {
                span.bbox = rule.bbox, span.HH = h + d
            }
            return rule;
        },
        ...
        placeBox: function( span, x, y, noclip )
        {
            ...
            if (this.msiePlaceBoxBug) {
                this.addText( span, this.NBSP )
            }
            if (this.imgSpaceBug) {
                this.addText( span, this.imgSpace )
            }
            // Place the box
            var HH, dx = 0;
            if (span.HH != null) {
            ...
            if (bbox) {
                if (this.initialSkipBug) {
                    if (bbox.lw < 0) {
                        dx = bbox.lw;
                        HTMLCSS.createBlank( span, -dx, true )
                    }
                    if (bbox.rw > bbox.w) {
                        HTMLCSS.createBlank( span, bbox.rw - bbox.w + .1 )
                    }
                }
            ...
            if (bbox && BBOX) {
                if (bbox.H != null && (BBOX.H == null || bbox.H + y > BBOX.H)) {
                    BBOX.H = bbox.H + y
                }
                if (bbox.D != null && (BBOX.D == null || bbox.D - y > BBOX.D)) {
                    BBOX.D = bbox.D - y
                }
                if (bbox.h + y > BBOX.h) {
                    BBOX.h = bbox.h + y
                ...
                if (bbox.w + x > BBOX.w) {
                    BBOX.w = bbox.w + x;
                    if (BBOX.width == null) {
                        parent.style.width = this.Em( BBOX.w )
                    }
                }
                if (bbox.rw + x > BBOX.rw) {
                    BBOX.rw = bbox.rw + x
                }
                if (bbox.lw + x < BBOX.lw) {
                    BBOX.lw = bbox.lw + x
                }
                if (bbox.width != null && !bbox.isFixed) {
                    if (BBOX.width == null) {
                        ...
                        if (bbox.minWidth) {
                            parent.style.minWidth = BBOX.minWidth = bbox.minWidth
                        }
                    }
        ...
        setStackWidth: function( span, w )
        {
            if (typeof(w) === "number") {
                ...
                if (span.parentNode.bbox) {
                    span.parentNode.bbox.width = w
                }
            }
        },
        ...
        createDelimiter: function( span, code, HW, scale, font )
        {
            ...
            if (delim.stretch) {
                this["extendDelimiter" + delim.dir]( span, hw, delim.stretch, scale, font )
            }
        },
        extendDelimiterV: function( span, H, delim, scale, font )
        {
            ...
            var y = -top.bbox.h;
            this.placeBox( top, 0, y, true );
            y -= top.bbox.d;
            if (delim.mid) {
                mid = this.createBox( stack );
                this.createChar( mid, delim.mid, scale, font );
            ...
            if (H > h) {
                ext = this.Element( "span" );
                this.createChar( ext, delim.ext, scale, font );
                ...
                while (k-- > 0) {
                    while (n-- > 0) {
                        ...
                        EXT.bbox = ext.bbox;
                        y -= eh;
                        this.placeBox( this.addBox( stack, EXT ), 0, y, true );
                    }
                    y += dy - ext.bbox.d;
                    if (delim.mid && k) {
                        this.placeBox( mid, 0, y - mid.bbox.h, true );
                        n = N;
                ...
                if (delim.mid) {
                    this.placeBox( mid, 0, y - mid.bbox.h, true );
                    y += -(mid.bbox.h + mid.bbox.d)
                }
                y += (h - H) / 2;
            }
            this.placeBox( bot, 0, y - bot.bbox.h, true );
            y -= bot.bbox.h + bot.bbox.d;
            span.bbox = {
        ...
        extendDelimiterH: function( span, W, delim, scale, font )
        {
            ...
            var rep = this.Element( "span" );
            this.createChar( rep, delim.rep, scale, font );
                ...
                while (k-- > 0) {
                    while (n-- > 0) {
                        if (!this.cloneNodeBug) {
                            REP = rep.cloneNode( true )
                        }
                        else {
                            REP = this.Element( "span" );
                            this.createChar( REP, delim.rep, scale, font )
                        }
                        REP.bbox = rep.bbox;
                        this.placeBox( this.addBox( stack, REP ), x, 0, true );
                        x += rw;
                    }
                    if (delim.mid && k) {
                        this.placeBox( mid, x, 0, true );
                        x += mid.bbox.w - dx;
                        n = N
                    }
                }
        ...
        createChar: function( span, data, scale, font )
        {
            ...
            if (data[2]) {
                span.style.marginLeft = this.Em( data[2] )
            }     // x offset
            if (data[3]) {                                              // y offset
        ...
        handleVariant: function( span, variant, text )
        {
            ...
            for (var i = 0, m = text.length; i < m; i++) {
                ...
                if (force || (!this.checkFont( font, SPAN.style ) && !c[5].img)) {
                    if (newtext.length) {
                        this.addText( SPAN, newtext );
                        newtext = ""
                    }
                    var addSpan = !!SPAN.style.fontFamily || !!span.style.fontStyle || !!span.style.fontWeight || !font.directory || force;
                    force = false;
                    if (SPAN !== span) {
                        addSpan = !this.checkFont( font, span.style );
                        SPAN = span
                    }
                    if (addSpan) {
                        SPAN = this.addElement( span, "span", {isMathJax: true, subSpan: true} )
                    }
                    this.handleFont( SPAN, font, SPAN !== span );
                }
                newtext = this.handleChar( SPAN, font, c, n, newtext );
                if (!(c[5] || {}).space) {
                    if (c[0] / 1000 > span.bbox.h) {
                        span.bbox.h = c[0] / 1000
                    }
                    if (c[1] / 1000 > span.bbox.d) {
                        span.bbox.d = c[1] / 1000
                    }
                }
                if (span.bbox.w + c[3] / 1000 < span.bbox.lw) {
                    span.bbox.lw = span.bbox.w + c[3] / 1000
                }
                if (span.bbox.w + c[4] / 1000 > span.bbox.rw) {
                    span.bbox.rw = span.bbox.w + c[4] / 1000
                }
            ...
            if (span.scale && span.scale !== 1) {
                span.bbox.h *= span.scale;
                span.bbox.d *= span.scale;
                span.bbox.w *= span.scale;
                span.bbox.lw *= span.scale;
                span.bbox.rw *= span.scale;
            }
            if (text.length == 1 && font.skew && font.skew[n]) {
                span.bbox.skew = font.skew[n]
            }
        },
        checkFont: function( font, style )
        {
            ...
            if (weight.match( /^\d+$/ )) {
                weight = (parseInt( weight ) >= 600 ? "bold" : "normal")
            }
        ...
        handleFont: function( span, font, force )
        {
            ...
            if (!(HTMLCSS.FontFaceBug && font.isWebFont)) {
                ...
                if (style !== "normal" || force) {
                    span.style.fontStyle = style
                }
                if (weight !== "normal" || force) {
        ...
        handleChar: function( span, font, c, n, text )
        {
            ...
            if (C.space) {
                if (text.length) {
                    this.addText( span, text )
                }
                HTMLCSS.createShift( span, c[2] / 1000 );
                return "";
            }
            if (C.img) {
                return this.handleImg( span, font, c, n, text )
            }
            if (C.isUnknown && this.FONTDATA.DELIMITERS[n]) {
                ...
                span.scale = scale;
                c[0] = span.bbox.h * 1000;
                c[1] = span.bbox.d * 1000;
                c[2] = span.bbox.w * 1000;
                c[3] = span.bbox.lw * 1000;
                c[4] = span.bbox.rw * 1000;
                return "";
            }
            if (C.c == null) {
                if (n <= 0xFFFF) {
            ...
            if (C.rfix) {
                this.addText( span, text + C.c );
                HTMLCSS.createShift( span, C.rfix / 1000 );
                return ""
            }
            if (c[2] || !this.msieAccentBug || text.length) {
                return text + C.c
            }
        ...
        {
            return text
        }, // replaced by imageFont extension
        ...
        unknownChar: function( variant, n )
        {
            ...
            if (!unknown[n]) {
                unknown[n] = [800, 200, 500, 0, 500, {isUnknown: true}]
            } // [h,d,w,lw,rw,{data}]
        ...
        loadFont: function( file )
        {
            ...
            if (this.imgFonts) {
                if (!MathJax.isPacked) {
                    file = file.replace( /\/([^\/]*)$/, HTMLCSS.imgPacked + "/$1" )
                }
    ...
    MathJax.Hub.Register.StartupHook( "mml Jax Ready", function()
    {
        ...
        MML.mbase.Augment( {
            toHTML: function( span )
            {
                ...
                for (i = 0, m = stretchy.length; i < m; i++) {
                    ...
                    if (stretchy[i].forceStretch || bbox.h !== h || bbox.d !== d) {
                        stretchy[i].HTMLstretchV( span, h, d );
                        stretched = true
                    }
                }
                if (stretched) {
                ...
                if (this.HTMLlineBreaks( span )) {
                    span = this.HTMLmultiline( span )
                }
            ...
            {
                MML.mbase.HTMLautoloadFile( "multiline" )
            },
            ...
            HTMLcombineBBoxes: function( core, BBOX )
            {
                ...
                if (child.style.paddingLeft) {
                    BBOX.w += HTMLCSS.unEm( child.style.paddingLeft ) * (child.scale || 1)
                }
                if (BBOX.w + bbox.lw < BBOX.lw) {
                    BBOX.lw = BBOX.w + bbox.lw
                }
                if (BBOX.w + bbox.rw > BBOX.rw) {
                    BBOX.rw = BBOX.w + bbox.rw
                }
                BBOX.w += bbox.w;
                if (child.style.paddingRight) {
                    BBOX.w += HTMLCSS.unEm( child.style.paddingRight ) * (child.scale || 1)
                }
                if (bbox.width) {
                    BBOX.width = bbox.width;
                    BBOX.minWidth = bbox.minWidth
                }
                if (bbox.tw) {
                    BBOX.tw = bbox.tw
                }
            ...
            HTMLcanStretch: function( direction )
            {
                if (this.isEmbellished()) {
                    var core = this.Core();
                    if (core && core !== this) {
                        return core.HTMLcanStretch( direction )
                    }
                }
                return false;
            },
            ...
            {
                if (this.data[n]) {
                    HTMLCSS.Measured( this.data[n].toHTML( box ), box )
                }
                else {
            ...
            HTMLcreateSpan: function( span )
            {
                if (this.spanID) {
                    ...
                    if (SPAN && (SPAN.parentNode === span || (SPAN.parentNode || {}).parentNode === span)) {
                        ...
                        SPAN.scale = 1;
                        SPAN.isMultChar = SPAN.HH = null;
                        SPAN.style.cssText = "";
                        return SPAN;
                    }
                }
                if (this.href) {
                    span = HTMLCSS.addElement( span, "a", {href: this.href, isMathJax: true} )
                }
                span = HTMLCSS.addElement( span, "span", {className: this.type, isMathJax: true} );
                if (HTMLCSS.imgHeightBug) {
                    span.style.display = "inline-block"
                }
                if (this["class"]) {
                ...
                if (this.style) {
                    ...
                    if (span.style.fontSize) {
                        this.mathsize = span.style.fontSize;
                        span.style.fontSize = ""
                    }
            ...
            HTMLhandleVariant: function( span, variant, text )
            {
                HTMLCSS.handleVariant( span, variant, text )
            },
            ...
            HTMLhandleSize: function( span )
            {
                if (!span.scale) {
                    ...
                    if (span.scale !== 1) {
                        span.style.fontSize = HTMLCSS.Percent( span.scale )
                    }
                }
                return span;
            },
            ...
            HTMLgetScale: function()
            {
                ...
                if (this.style) {
                    var span = this.HTMLspanElement();
                    if (span.style.fontSize != "") {
                        values.fontsize = span.style.fontSize
                    }
                }
                ...
                this.scale = scale;
                this.mscale = HTMLCSS.length2em( values.mathsize );
            ...
            HTMLgetVariant: function()
            {
                ...
                if (this.style) {
                    var span = this.HTMLspanElement();
                    if (!values.family && span.style.fontFamily) {
                        values.family = span.style.fontFamily
                    }
                    if (!values.weight && span.style.fontWeight) {
                        values.weight = span.style.fontWeight
                    }
                    if (!values.style && span.style.fontStyle) {
                        values.style = span.style.fontStyle
                    }
                }
                ...
                if (this.variantForm) {
                    variant = "-" + HTMLCSS.fontInUse + "-variant"
                }
        ...
        MML.chars.Augment( {
            toHTML: function( span, variant, remap, chars )
            {
                ...
                if (variant.fontInherit) {
                    ...
                    if (variant.bold) {
                        span.lastChild.style.fontWeight = "bold"
                    }
                    if (variant.italic) {
                        span.lastChild.style.fontStyle = "italic"
                    }
                    span.bbox = null;
        ...
        MML.mi.Augment( {
            toHTML: function( span )
            {
                ...
                for (var i = 0, m = this.data.length; i < m; i++) {
                    if (this.data[i]) {
                        this.data[i].toHTML( span, variant )
                    }
                }
                if (!span.bbox) {
                    span.bbox = this.HTMLzeroBBox()
        ...
        MML.mo.Augment( {
            toHTML: function( span )
            {
                ...
                if (text.length === 1 && parent && parent.isa( MML.munderover ) &&
                    ...
                    if (over && this === over.CoreMO() && parent.Get( "accent" )) {
                        mapchars = HTMLCSS.FONTDATA.REMAPACCENT
                    }
                    else {
                        if (under && this === under.CoreMO() && parent.Get( "accentunder" )) {
                            mapchars = HTMLCSS.FONTDATA.REMAPACCENTUNDER
                        }
                    }
                }
                //
                ...
                for (var i = 0, m = this.data.length; i < m; i++) {
                    if (this.data[i]) {
                        this.data[i].toHTML( span, variant, this.remap, mapchars )
                    }
                }
                if (!span.bbox) {
                    span.bbox = this.HTMLzeroBBox()
                ...
                if (values.largeop) {
                    ...
                    if (HTMLCSS.safariVerticalAlignBug && span.lastChild.nodeName === "IMG") {
                        ...
                        if (HTMLCSS.konquerorVerticalAlignBug && span.lastChild.nodeName === "IMG") {
                            span.style.position = "relative";
            ...
            HTMLcanStretch: function( direction )
            {
                ...
                if (parent && parent.isa( MML.munderover ) &&
                    ...
                    var over = parent.data[parent.over], under = parent.data[parent.under];
                    if (over && this === over.CoreMO() && parent.Get( "accent" )) {
                        c = HTMLCSS.FONTDATA.REMAPACCENT[c] || c
                    }
                    else {
                        if (under && this === under.CoreMO() && parent.Get( "accentunder" )) {
                            c = HTMLCSS.FONTDATA.REMAPACCENTUNDER[c] || c
                        }
                    }
                }
            ...
            HTMLstretchH: function( box, W )
            {
                ...
                if ((values.fontweight === "bold" || parseInt( values.fontweight ) >= 600) && !this.Get( "mathvariant",
                        true )) {
        ...
        MML.mtext.Augment( {
            toHTML: function( span )
            {
                ...
                if (HTMLCSS.config.mtextFontInherit || this.Parent().type === "merror") {
                    ...
                    if (vname === "monospace") {
                        span.className += " MJX-monospace"
                    }
                    else {
                        if (vname.match( /sans-serif/ )) {
                            span.className += " MJX-sans-serif"
                        }
                    }
        ...
        MML.mphantom.Augment( {
            toHTML: function( span, HW, D )
            {
                ...
                if (this.data[0] != null) {
                    ...
                    if (D != null) {
                        HTMLCSS.Remeasured( this.data[0].HTMLstretchV( span, HW, D ), span )
                    }
                    else {
                        if (HW != null) {
                            HTMLCSS.Remeasured( this.data[0].HTMLstretchH( span, HW ), span )
                        }
                        else {
                            box = HTMLCSS.Measured( box, span )
                        }
                    }
        ...
        MML.mpadded.Augment( {
            toHTML: function( span, HW, D )
            {
                ...
                if (this.data[0] != null) {
                    ...
                    if (D != null) {
                        HTMLCSS.Remeasured( this.data[0].HTMLstretchV( box, HW, D ), box )
                    }
                    else {
                        if (HW != null) {
                            HTMLCSS.Remeasured( this.data[0].HTMLstretchH( box, HW ), box )
                        }
                        else {
                    ...
                    this.HTMLgetScale();
                    if (values.lspace) {
                        x = this.HTMLlength2em( box, values.lspace, mu )
                    }
                    if (values.voffset) {
                        y = this.HTMLlength2em( box, values.voffset, mu )
                    }
                    ...
                    if (values.height !== "") {
                        span.bbox.h = this.HTMLlength2em( box, values.height, mu, "h", 0 )
                    }
                    if (values.depth !== "") {
                        span.bbox.d = this.HTMLlength2em( box, values.depth, mu, "d", 0 )
                    }
                    if (values.width !== "") {
                        span.bbox.w = this.HTMLlength2em( box, values.width, mu, "w", 0 )
                    }
            ...
            HTMLlength2em: function( span, length, mu, d, m )
            {
                ...
                if (d && String( length ).match( /^\s*[-+]/ )) {
                    return Math.max( m, span.bbox[d] + v )
                } else {
                    return v
                }
            },
        ...
        MML.mstyle.Augment( {
            toHTML: function( span, HW, D )
            {
                ...
                if (this.data[0] != null) {
                    ...
                    if (D != null) {
                        this.data[0].HTMLstretchV( span, HW, D )
                    }
                    else {
                        if (HW != null) {
                            this.data[0].HTMLstretchH( span, HW )
                        }
                    }
                    span.bbox = SPAN.bbox;
                }
        ...
        MML.mfrac.Augment( {
            toHTML: function( span )
            {
                ...
                if (values.bevelled) {
                    ...
                    HTMLCSS.placeBox( den, num.bbox.w + bevel.bbox.w - delta,
                        (den.bbox.d - den.bbox.h) / 2 + a - delta );
                } else {
                    ...
                    if (isDisplay) {
                        u = HTMLCSS.TeX.num1;
                        v = HTMLCSS.TeX.denom1
                    }
                    else {
                        u = (t === 0 ? HTMLCSS.TeX.num3 : HTMLCSS.TeX.num2);
                        v = HTMLCSS.TeX.denom2
                    }
                    u *= scale;
                    v *= scale;
                    if (t === 0) {// \atop
                        ...
                        q = (u - num.bbox.d) - (a + t / 2);
                        if (q < p) {
                            u += p - q
                        }
                        q = (a - t / 2) - (den.bbox.h - v);
                        if (q < p) {
                            v += p - q
                        }
        ...
        MML.msqrt.Augment( {
            toHTML: function( span )
            {
                ...
                W = BASE.bbox.w;
                var x = 0;
                if (surd.isMultiChar || (HTMLCSS.AdjustSurd && HTMLCSS.imgFonts)) {
                    surd.bbox.w *= .95
                }
                if (surd.bbox.h + surd.bbox.d > H) {
                    q = ((surd.bbox.h + surd.bbox.d) - (H - t)) / 2
                }
        ...
        MML.mfenced.Augment( {
            toHTML: function( span )
            {
                ...
                for (var i = 1, m = this.data.length; i < m; i++) {
                    if (this.data[i]) {
                        if (this.data["sep" + i]) {
                            this.data["sep" + i].toHTML( span )
                        }
                        this.data[i].toHTML( span );
                    }
                }
                if (this.data.close) {
                    this.data.close.toHTML( span )
                }
                var stretchy = this.HTMLcomputeBBox( span );
            ...
            toHTML: function( span, HW, D )
            {
                ...
                for (i = 0, m = this.data.length; i < m; i++) {
                    if (this.data[i] != null) {
                        ...
                        if (i == this.base) {
                            if (D != null) {
                                this.data[this.base].HTMLstretchV( box, HW, D )
                            }
                            else {
                                if (HW != null) {
                                    this.data[this.base].HTMLstretchH( box, HW )
                                }
                            }
                ...
                for (i = 0, m = this.data.length; i < m; i++) {
                    if (this.data[i] != null) {
                        ...
                        if (accent && box.bbox.w <= 1 / HTMLCSS.em + .0001) { // images can get the width off by 1px
                            box.bbox.w = box.bbox.rw - box.bbox.lw;
                            box.bbox.noclip = true;
                            if (box.bbox.lw) {
                        ...
                        if (i == this.over) {
                            if (accent) {
                                k = Math.max( t * scale * factor, 2.5 / this.em );
                                z3 = 0;
                                if (base.bbox.skew) {
                                    x += base.bbox.skew;
                                    span.bbox.skew = base.bbox.skew;
                                    if (x + box.bbox.w > WW) {
                                        span.bbox.skew += (WW - box.bbox.w - x) / 2
                                    }
                                }
                            } else {
        ...
        MML.msubsup.Augment( {
            toHTML: function( span, HW, D )
            {
                ...
                if (this.data[this.base]) {
                    ...
                    if (D != null) {
                        this.data[this.base].HTMLstretchV( base, HW, D )
                    }
                    else {
                        if (HW != null) {
                            this.data[this.base].HTMLstretchH( base, HW )
                        }
                    }
                } else {
                    base.bbox = this.HTMLzeroBBox()
                }
                ...
                if (this.HTMLnotEmpty( this.data[this.sup] )) {
                    ...
                    children.push( this.data[this.sup].toHTML( sup ) )
                }
                if (this.HTMLnotEmpty( this.data[this.sub] )) {
                    sub = HTMLCSS.createBox( stack );
                    children.push( this.data[this.sub].toHTML( sub ) )
                }
                HTMLCSS.MeasureSpans( children );
                if (sup) {
                    sup.bbox.w += s;
                ...
                if (this.data[this.base] && HW == null && D == null &&
                    ...
                    if (this.data[this.base].data.join( "" ).length === 1 && children[0].scale === 1 && !this.data[this.base].Get( "largeop" )) {
                        u = v = 0
                    }
                }
        ...
        MML.annotation.Augment( {
            toHTML: function( span )
            {
                return this.HTMLcreateSpan( span )
            }
        } );
        ...
        MML.math.Augment( {
            toHTML: function( span, node )
            {
                ...
                if (alttext && !span.getAttribute( "aria-label" )) {
...
//      span.setAttribute("tabindex",0);  // causes focus outline, so disable for now
                ...
                stack.style.fontSize = nobr.parentNode.style.fontSize;
                nobr.parentNode.style.fontSize = "";
                if (this.data[0] != null) {
                    ...
                    var html = this.data[0].toHTML( box );
                    html.bbox.exactW = false; // force remeasure just to be sure
                ...
                //
                span.style.width = HTMLCSS.Em( Math.max( 0,
                    Math.round( math.bbox.w * this.em ) + .25 ) / HTMLCSS.outerEm );
                span.style.display = "inline-block";
                //
                //  Adjust bbox to match outer em-size
                //
                var p = 1 / HTMLCSS.em, f = HTMLCSS.em / HTMLCSS.outerEm;
                HTMLCSS.em /= f;
                span.bbox.h *= f;
                span.bbox.d *= f;
                span.bbox.w *= f;
                ...
                //
                if (math) {
                    HTMLCSS.createRule( span, (math.bbox.h + p) * f, (math.bbox.d + p) * f, 0 )
                }
                //
                ...
                if (!this.isMultiline && this.Get( "display" ) === "block" && span.bbox.width == null) {
                    ...
                    if (shift) {
                        ...
                        HUB.Insert( span.style, ({
                            ...
                            right: {
                                marginLeft: HTMLCSS.Em( Math.max( 0, span.bbox.w + shift ) ),
                                marginRight: HTMLCSS.Em( -shift )
                            },
                        ...
                        if (color) {
                            ...
                            color.style.marginRight =
                                ...
                                + (values.indentalign === "right" ? Math.min( 0,
            ...
            toHTML: function( span, HW, D )
            {
                ...
                if (this.data[0] != null) {
                    if (this.texClass === MML.TEXCLASS.VCENTER) {
                        ...
                        if (D != null) {
                            html = this.data[0].HTMLstretchV( box, HW, D )
                        }
                        else {
                            if (HW != null) {
                                html = this.data[0].HTMLstretchH( box, HW )
                            }
                        }
                        span.bbox = html.bbox;
                    }
        ...
        HUB.Browser.Select( {
            ...
            Safari: function( browser )
            {
                ...
                if (forceImages) {
                    ...
                    if (config) {
                        config.availableFonts = [];
                        config.preferredFont = null
                    }
                    else {
                        HUB.config["HTML-CSS"] = {availableFonts: [], preferredFont: null}
                    }
                }
            },
