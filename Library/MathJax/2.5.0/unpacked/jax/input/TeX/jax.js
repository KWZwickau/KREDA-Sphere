        ...
        checkLines: function()
        {
            if (this.arraydef.rowlines) {
                ...
                if (lines.length === this.table.length) {
                    ...
                    if (lines.length < this.table.length - 1) {
            ...
            for (var id in src) {
                if (src.hasOwnProperty( id )) {
                    if (typeof src[id] === 'object' && !(src[id] instanceof Array) &&
                        ...
                        if (!dst[id] || !dst[id].isUser || !nouser) {
                            dst[id] = src[id]
                        }
                    }
                }
...
//      noalign:            'HandleNoAlign',
    ...
    var PARSE = MathJax.Object.Subclass( {
        ...
        Parse: function()
        {
            ...
            while (this.i < this.string.length) {
                ...
                if (TEXDEF.special[c]) {
                    ...
                    if (TEXDEF.letter.test( c )) {
                        this.Variable( c )
                    }
                    else {
                        if (TEXDEF.digit.test( c )) {
                            this.Number( c )
                        }
                        else {
                            this.Other( c )
        ...
        FBox: function( name )
        {
            this.Push( MML.menclose.apply( MML,
                this.InternalMath( this.GetArgument( name ) ) ).With( {notation: "box"} ) );
        },
        ...
        HFill: function( name )
        {
            ...
            if (top.isa( STACKITEM.array )) {
                ...
                TEX.Error( ["UnsupportedHFill", "Unsupported use of %1", name] );
            }
        },
        ...
        Array: function( begin, open, close, align, spacing, vspacing, style, raggedHeight )
        {
            ...
            if (lines.match( /[|:]/ )) {
                ...
                array.arraydef.columnlines =
                    lines.split( '' ).join( ' ' ).replace( /[^|: ]/g, 'none' ).replace( /\|/g, 'solid' ).replace( /:/g,
                        'dashed' );
            }
            if (open) {
            ...
            while (i < text.length) {
                ...
                if (c === '$') {
                    if (match === '$') {
                        ...
                        if (match === '') {
                            if (k < i - 1) {
                                mml.push( this.InternalText( text.slice( k, i - 1 ), def ) )
                            }
                            match = '$';
                            k = i;
                        }
                    ...
                    if (c === '}' && match === '}') {
                        ...
                        if (c === '\\') {
                            if (match === '' && text.substr( i ).match( /^(eq)?ref\s*\{/ )) {
                                ...
                                if (c === '(' && match === '') {
                                    if (k < i - 2) {
                                        mml.push( this.InternalText( text.slice( k, i - 2 ), def ) )
                                    }
                                    match = ')';
                                    k = i;
                                } else {
                                    if (c === ')' && match === ')') {
                                        mml.push( MML.TeXAtom( TEX.Parse( text.slice( k, i - 2 ),
                                            {} ).mml().With( def ) ) );
                                        match = '';
                                        k = i;
                                    } else {
    ...
    TEX.Augment( {
        ...
        Translate: function( script )
        {
            ...
            if (mml.isa( MML.mtable ) && mml.displaystyle === "inherit") {
