...
(function( HUB, HTML, AJAX, CALLBACK, OUTPUT )
{
    ...
    var MENU = MathJax.Menu = MathJax.Object.Subclass( {
        ...
        Event: function( event, menu, type, force )
        {
            ...
            if (MENU.skipUp) {
                ...
                if (type === "Touchstart" ||
                    (type === "Mousedown" && !MENU.skipMousedown)) {
                    delete MENU.skipUp
                }
            }
            if (!event) {
        ...
        Mouseover: function( event, menu )
        {
            ...
            if (!this.menu || !this.menu.posted) {
                ...
                for (var i = 0, m = items.length; i < m; i++) {
                    ...
                    if (item && item.menu && item.menu.posted) {
    ...
    MENU.ITEM.LABEL = MENU.ITEM.Subclass( {
        ...
        Label: function( def, menu )
        {
            delete def.onmouseover, delete def.onmouseout;
            delete def.onmousedown;
    ...
    MENU.ShowSource = function( event )
    {
        ...
        if (this.format === "MathML") {
            ...
            if (this.format === "Error") {
                ...
                if (CONFIG.semanticsAnnotations[this.format]) {
    ...
    MENU.ShowSource.Text = function( text, event )
    {
        ...
        if (MENU.isMobile) {
            ...
            w.document.write( "<hr><input type='button' value='" + _( "Close",
    ...
    MENU.MPEvents = function( item )
    {
        ...
        if (!isIE9) {
            ...
            if (!discoverable && item.name[1] === "Menu Events" && CONFIG.settings.mpContext) {
    ...
    MENU.CreateAnnotationMenu = function()
    {
        ...
        for (var a in annotations) {
            if (annotations.hasOwnProperty( a )) {
                menu.items.push( ITEM.COMMAND( [a, a], MENU.ShowSource,
                    {hidden: true, nativeTouch: true, format: a} ) );
            }
        }
    };
    ...
    HUB.Register.StartupHook( "End Config", function()
    {
        ...
        MENU.menu = MENU(
            ...
            ITEM.SUBMENU( ["Settings", "Math Settings"],
                ...
                ITEM.SUBMENU( "MathPlayer", {
                        hidden: !HUB.Browser.isMSIE || !CONFIG.showMathPlayer,
                        disabled: !HUB.Browser.hasMathPlayer
                    },
                    ITEM.LABEL( ["MPHandles", "Let MathPlayer Handle:"] ),
                    ITEM.CHECKBOX( ["MenuEvents", "Menu Events"], "mpContext",
                        {action: MENU.MPEvents, hidden: !isIE9} ),
                    ITEM.CHECKBOX( ["MouseEvents", "Mouse Events"], "mpMouse",
                        {action: MENU.MPEvents, hidden: !isIE9} ),
                    ITEM.CHECKBOX( ["MenuAndMouse", "Mouse and Menu Events"], "mpMouse",
                        {action: MENU.MPEvents, hidden: isIE9} )
                ),
                ...
                ITEM.CHECKBOX( ["Discoverable", "Highlight on Hover"], "discoverable",
