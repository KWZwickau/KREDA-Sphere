...
//    ddddot:     ['Macro','\\mathop{#1}\\limits^{\\textstyle \\mathord{.}\\mathord{.}\\mathord{.}\\mathord{.}}',1],
            ...
            varinjlim: [
                'Macro',
                '\\mathop{\\underrightarrow{\\mmlToken{mi}{lim}\\Rule{-1pt}{0pt}{1pt}}\\Rule{0pt}{0pt}{.45em}}'
            ],
            varprojlim: [
                'Macro',
                '\\mathop{\\underleftarrow{\\mmlToken{mi}{lim}\\Rule{-1pt}{0pt}{1pt}}\\Rule{0pt}{0pt}{.45em}}'
            ],
    ...
    TEX.Parse.Augment( {
        ...
        Genfrac: function( name, left, right, thick, style )
        {
            if (left == null) {
                ...
                left = this.convertDelimiter( left )
            }
            if (right == null) {
                right = this.GetDelimiterArg( name )
            } else {
                right = this.convertDelimiter( right )
            }
            if (thick == null) {
    ...
    STACKITEM.Augment( {
        ...
        getTag: function()
        {
            var global = this.global, tag = global.tag;
            global.tagged = true;
            if (global.label) {
                ...
                if (global.tag) {
                    ...
                    if (mml.displayAlign === MML.INDENTALIGN.LEFT) {
                        ...
                        if (mml.displayIndent !== "0") {
                            def.columnwidth = mml.displayIndent + " fit";
                            def.columnspacing = "0";
                            row = [row[0], MML.mtd(), row[1]];
                        }
                    } else {
                        if (mml.displayAlign === MML.INDENTALIGN.RIGHT) {
                            def.width = "100%";
                            if (mml.displayIndent !== "0") {
                                def.columnwidth = "fit " + mml.displayIndent;
                                def.columnspacing = "0";
                                row[2] = MML.mtd();
                            }
                        }
