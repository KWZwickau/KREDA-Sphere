$( function()
{
    ...
    QUnit.test( 'should add data attribute for referencing original title', function( assert )
    {
        assert.expect( 1 );
        var $trigger = $( '<a href="#" rel="tooltip" title="Another tooltip"/>' ).bootstrapTooltip();
        assert.strictEqual( $trigger.attr( 'data-original-title' ), 'Another tooltip',
    ...
    QUnit.test( 'should add aria-describedby to the trigger on show', function( assert )
    {
        ...
        assert.strictEqual( $( '.tooltip' ).attr( 'aria-describedby' ), $trigger.attr( 'id' ),
    ...
    QUnit.test( 'should destroy tooltip', function( assert )
    {
        ...
        assert.ok( $._data( $tooltip[0], 'events' ).mouseover && $._data( $tooltip[0], 'events' ).mouseout,
            'tooltip has hover events' );
        assert.strictEqual( $._data( $tooltip[0], 'events' ).click[0].namespace, 'foo',
    ...
    QUnit.test( 'should add position class before positioning so that position-specific styles are taken into account',
        function( assert )
        {
            assert.expect( 1 );
            var styles = '<style>'
    ...
    QUnit.test( 'should use title attribute for tooltip text', function( assert )
    {
        ...
        $tooltip.bootstrapTooltip( 'show' );
        assert.strictEqual( $( '.tooltip' ).children( '.tooltip-inner' ).text(), 'Simple tooltip',
    ...
    QUnit.test( 'should use title option', function( assert )
    {
        ...
        $tooltip.bootstrapTooltip( 'show' );
        assert.strictEqual( $( '.tooltip' ).children( '.tooltip-inner' ).text(), 'This is a tooltip with some content',
    ...
    QUnit.test( 'should position tip on top if viewport has enough space and placement is "auto top"',
        function( assert )
        {
            assert.expect( 2 );
            var styles = '<style>'
    ...
    QUnit.test( 'should position tip on bottom if the tip\'s dimension exceeds the viewport area and placement is "auto top"',
        function( assert )
        {
            assert.expect( 2 );
            var styles = '<style>'
    ...
    QUnit.test( 'should display the tip on top whenever scrollable viewport has enough room if the given placement is "auto top"',
        function( assert )
        {
            assert.expect( 2 );
            var styles = '<style>'
    ...
    QUnit.test( 'should display the tip on bottom whenever scrollable viewport doesn\'t have enough room if the given placement is "auto top"',
        function( assert )
        {
            assert.expect( 2 );
            var styles = '<style>'
    ...
    QUnit.test( 'should display the tip on bottom whenever scrollable viewport has enough room if the given placement is "auto bottom"',
        function( assert )
        {
            assert.expect( 2 );
            var styles = '<style>'
    ...
    QUnit.test( 'should display the tip on top whenever scrollable viewport doesn\'t have enough room if the given placement is "auto bottom"',
        function( assert )
        {
            assert.expect( 2 );
            var styles = '<style>'
    ...
    QUnit.test( 'should adjust the tip\'s top position when up against the bottom of the viewport', function( assert )
    {
        ...
        $target.bootstrapTooltip( 'show' );
        var $tooltip = $container.find( '.tooltip' );
        assert.strictEqual( Math.round( $tooltip.offset().top ),
            Math.round( $( window ).height() - 12 - $tooltip[0].offsetHeight ) );
    ...
    QUnit.test( 'should adjust the tip\'s left position when up against the right of the viewport', function( assert )
    {
        ...
        $target.bootstrapTooltip( 'show' );
        var $tooltip = $container.find( '.tooltip' );
        assert.strictEqual( Math.round( $tooltip.offset().left ),
            Math.round( $( window ).width() - 12 - $tooltip[0].offsetWidth ) );
    ...
    QUnit.test( 'should adjust the tip when up against the right of an arbitrary viewport', function( assert )
    {
        ...
        var $tooltip = $container.find( '.tooltip' );
        assert.strictEqual( Math.round( $tooltip.offset().left ),
            Math.round( 60 + $container.width() - $tooltip[0].offsetWidth ) );
    ...
    QUnit.test( 'should not error when trying to show an auto-placed tooltip that has been removed from the dom',
        function( assert )
        {
            assert.expect( 1 );
            var passed = true;
    ...
    QUnit.test( 'should not hide tooltip if leave event occurs and enter event occurs within the hide delay',
        function( assert )
        {
            assert.expect( 3 );
            var done = assert.async();
    ...
    QUnit.test( 'should not show tooltip if leave event occurs before delay expires, even if hide delay is 0',
        function( assert )
        {
            assert.expect( 2 );
            var done = assert.async();
    ...
    QUnit.test( 'should correctly determine auto placement based on container rather than parent', function( assert )
    {
        ...
        $trigger
            .on( 'shown.bs.tooltip', function()
            {
                ...
                assert.ok( tipXrightEdge < triggerXleftEdge,
    ...
    QUnit.test( 'should throw an error when initializing tooltip on the document object without specifying a delegation selector',
        function( assert )
        {
            assert.expect( 1 );
