$( function()
{
    ...
    QUnit.test( 'should get title and content from options', function( assert )
    {
        ...
        assert.strictEqual( $( '.popover .popover-content' ).text(), 'loves writing tests （╯°□°）╯︵ ┻━┻',
    ...
    QUnit.test( 'should get title and content from attributes', function( assert )
    {
        ...
        assert.strictEqual( $( '.popover .popover-content' ).text(), 'loves data attributes (づ｡◕‿‿◕｡)づ ︵ ┻━┻',
            'content correctly inserted' );
    ...
    QUnit.test( 'should destroy popover', function( assert )
    {
        ...
        assert.ok( $._data( $popover[0], 'events' ).mouseover && $._data( $popover[0], 'events' ).mouseout,
            'popover has hover event' );
        assert.strictEqual( $._data( $popover[0], 'events' ).click[0].namespace, 'foo',
    ...
    QUnit.test( 'should detach popover content rather than removing it so that event handlers are left intact',
        function( assert )
        {
            assert.expect( 1 );
    ...
    QUnit.test( 'should throw an error when initializing popover on the document object without specifying a delegation selector',
        function( assert )
        {
            assert.expect( 1 );
