$( function()
{
    ...
    QUnit.test( 'should set aria-hidden to false when show method is called', function( assert )
    {
        ...
        $( '<div id="modal-test"/>' )
            .on( 'shown.bs.modal', function()
            {
                assert.strictEqual( $( '#modal-test' ).attr( 'aria-hidden' ), 'false',
    ...
    QUnit.test( 'should set aria-hidden to true when hide is called', function( assert )
    {
        ...
        $( '<div id="modal-test"/>' )
            ...
            .on( 'hidden.bs.modal', function()
            {
                assert.strictEqual( $( '#modal-test' ).attr( 'aria-hidden' ), 'true',
    ...
    QUnit.test( 'should restore focus to toggling element when modal is hidden after having been opened via data-api',
        function( assert )
        {
            assert.expect( 1 );
            var done = assert.async();
            ...
            $( '<div id="modal-test"><div class="contents"><div id="close" data-dismiss="modal"/></div></div>' )
                .on( 'hidden.bs.modal', function()
                {
                    setTimeout( function()
                    {
                        assert.ok( $( document.activeElement ).is( $toggleBtn ),
    ...
    QUnit.test( 'should not restore focus to toggling element if the associated show event gets prevented',
        function( assert )
        {
            assert.expect( 1 );
            var done = assert.async();
    ...
    QUnit.test( 'should ignore other inline styles when trying to restore body padding after closing',
        function( assert )
        {
            assert.expect( 2 );
            var done = assert.async();
