...
// `QUnit` initialized at top of scope
    QUnit = {
        ...
        start: function( count )
        {
            ...
            if (config.semaphore < 0) {
                config.semaphore = 0;
                QUnit.pushFailure( "Called start() while already started (QUnit.config.semaphore was 0 already)", null,
        ...
        push: function( result, actual, expected, message )
        {
            ...
            if (!result) {
                ...
                if (actual !== expected) {
                    ...
                    output += "<tr class='test-diff'><th>Diff: </th><td><pre>" + QUnit.diff( expected,
                        actual ) + "</pre></td></tr>";
                }
    ...
    Test.prototype = {
        ...
        run: function()
        {
            ...
            try {
                ...
                QUnit.pushFailure( "Died on test #" + (this.assertions.length + 1) + " " + this.stack + ": " + ( e.message || e ),
...
// - config.current.assertions.push({ .. });
    ...
    QUnit.constructor.prototype.raises = function()
    {
        QUnit.push( false, false, false,
            "QUnit.raises has been deprecated since 2012 (fad3c1ea), use QUnit.throws instead" );
    };
                ...
                parsers: {
                    ...
                    object: function( map, stack )
                    {
                        ...
                        for (i = 0; i < keys.length; i++) {
                            key = keys[i];
                            val = map[key];
                            ret.push( QUnit.jsDump.parse( key, "key" ) + ": " + QUnit.jsDump.parse( val, undefined,
                                stack ) );
                        }
                        QUnit.jsDump.down();
                        return join( "{", ret, "}" );
