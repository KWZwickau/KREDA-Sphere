...
{
    expect( 3 );
    deepEqual( jQuery( "div p", "#qunit-fixture" ).get(), q( "sndp", "en", "sap" ),
        "Basic selector with string as context" );
    deepEqual( jQuery( "div p", q( "qunit-fixture" )[0] ).get(), q( "sndp", "en", "sap" ),
        "Basic selector with element as context" );
    deepEqual( jQuery( "div p", jQuery( "#qunit-fixture" ) ).get(), q( "sndp", "en", "sap" ),
...
{
    expect( 1 );
    deepEqual( jQuery( "#qunit-fixture p" ).get(), q( "firstp", "ap", "sndp", "en", "sap", "first" ),
        "Get All Elements" );
} );
...
test( "get(-Number)", function()
{
    ...
    strictEqual( jQuery( "#firstp" ).get( -2 ), undefined,
...
test( "jQuery.grep()", function()
{
    ...
    deepEqual( jQuery.grep( [1, 2, 3, 4, 5, 6], searchCriterion, true ), [1, 3, 5],
        "Satisfying elements present and grep inverted" );
    deepEqual( jQuery.grep( [1, 3, 5, 7], searchCriterion, true ), [1, 3, 5, 7],
        "Satisfying elements absent and grep inverted" );

    deepEqual( jQuery.grep( [1, 2, 3, 4, 5, 6], searchCriterion, false ), [2, 4, 6],
        "Satisfying elements present but grep explicitly uninverted" );
    deepEqual( jQuery.grep( [1, 3, 5, 7], searchCriterion, false ), [],
...
test( "jQuery.extend(Object, Object)", function()
{
    ...
    ok( jQuery.isPlainObject( jQuery.extend( true, {"arr": arr}, {"arr": {}} )["arr"] ),
...
test( "jQuery.makeArray", function()
{
    ...
    equal( jQuery.makeArray( document.getElementsByName( "PWD" ) ).slice( 0, 1 )[0].name, "PWD",
    ...
    equal( jQuery.makeArray( document.createElement( "div" ) )[0].nodeName.toUpperCase(), "DIV",
        "Pass makeArray a single node" );

    equal( jQuery.makeArray( {length: 2, 0: "a", 1: "b"} ).join( "" ), "ab",
        "Pass makeArray an array like map (with length)" );

    ok( !!jQuery.makeArray( document.documentElement.childNodes ).slice( 0, 1 )[0].nodeName,
        "Pass makeArray a childNodes array" );

    // function, is tricky as it has length
    equal( jQuery.makeArray( function()
    {
        return 1;
...
test( "jQuery.inArray", function()
{
    ...
    equal( jQuery.inArray( 0, undefined ), -1,
...
testIframeWithCallback( "Conditional compilation compatibility (#13274)", "core/cc_on.html",
    function( cc_on, errors, $ )
    {
        expect( 3 );
...
if (!/iphone os 7_/i.test( navigator.userAgent )) {
    testIframeWithCallback( "document ready when jQuery loaded asynchronously (#13655)", "core/dynamic_ready.html",
        function( ready )
        {
            expect( 1 );
