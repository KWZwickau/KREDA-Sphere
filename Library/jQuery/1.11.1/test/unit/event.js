...
test( "on(), with different this object", function()
{
    ...
    ok( !jQuery._data( jQuery( "#firstp" )[0], "events" ),
...
test( "trigger(type, [data], [fn])", function()
{
    ...
    pass = true;
    try {
        jQuery( "#qunit-fixture table" ).eq( 0 ).on( "test:test", function()
        {
        } ).trigger( "test:test" );
    } catch( e ) {
        pass = false;
...
if (window.onbeforeunload === null && !/(ipad|iphone|ipod|android 2\.3)/i.test( navigator.userAgent )) {
        ...
        iframe.appendTo( "#qunit-fixture" ).one( "load", function()
        {
            ...
            function checker()
            {
                ...
                iframe.appendTo( "#qunit-fixture" ).one( "load", function()
                {
                    ...
                    jQuery( win ).on( "beforeunload", function()
                    {
                        strictEqual( win.onbeforeunload, null,
                            "Event handler is fired, even when onbeforeunload property on window is nulled" );
...
test( ".on()/.off()", function()
{
    ...
    {
        equal( e.data, true, "on with event data" );
    } );
    jQuery( "#foo" ).trigger( "click" );
    ...
    {
        equal( data, true, "on with trigger data" );
    } );
    ...
    {
        equal( this["foo"], "bar", "on with event scope" );
    }, {"foo": "bar"} ) );
    jQuery( "#foo" ).trigger( "click" );
    jQuery( "#body" ).off( "click", "#foo" );
...
if (hasPHP) {
    testIframeWithCallback( "jQuery.ready synchronous load with long loading subresources", "event/syncReady.html",
        function( isOk )
        {
            expect( 1 );
...
test( "change handler should be detached from element", function()
{
    ...
    var $fixture = jQuery( "<input type='text' id='change-ie-leak' />" ).appendTo( "body" ),
        ...
        wrapperRemoveEvent = function( elem, type, handle )
        {
            ...
            equal( "change-ie-leak", jQuery( elem ).attr( "id" ),
