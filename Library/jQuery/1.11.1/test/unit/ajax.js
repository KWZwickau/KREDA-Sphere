...
//----------- jQuery.ajax()
    ...
    ajaxTest( "jQuery.ajax() - context modification through ajaxSetup", 3, function()
    {
        ...
        return {
            setup: function()
            {
                ...
                strictEqual( jQuery.ajaxSettings.context, obj,
            ...
            return {
                ...
                beforeSend: function()
                {
                    ...
                    while (( tmp = re.exec( this.url ) )) {
                        ...
                        parameter = tmp[1];
                        notStrictEqual( parameter, "tobereplaced555",
                            title + ": parameter (if it was there) was replaced" );
                    }
                    return false;
                },
                error: true
            ...
            requests: [
                {
                    ...
                    success: function( data )
                    {
                        ...
                        strictEqual( window["functionToCleanUp"], true,
                            "Callback was removed (GET, custom callback name to be cleaned up)" );
                        var xhr;
                        jQuery.ajax( {
                            url: "data/jsonp.php",
                            dataType: "jsonp",
                        ...
                        xhr.fail( function()
                        {
                            ...
                            strictEqual( window["functionToCleanUp"], true,
                                "Callback was removed after early abort (GET, custom callback name to be cleaned up)" );
                        } );
                    }
                }, {
    ...
    asyncTest( "jQuery.ajax() - statusText", 3, function()
    {
        jQuery.ajax( url( "data/statusText.php?status=200&text=Hello" ) ).done( function( _, statusText, jqXHR )
        {
            ...
            ok( jqXHR.statusText === "Hello" || jqXHR.statusText === "OK",
    ...
    jQuery.each( [" - Same Domain", " - Cross Domain"], function( crossDomain, label )
    {
        ajaxTest( "#8205 - jQuery.ajax() - JSONP - re-use callbacks name" + label, 2, {
            ...
            {
                var previous = this;
                strictEqual( previous.jsonpCallback, undefined,
    ...
    ajaxTest( "#11151 - jQuery.ajax() - parse error body", 2, {
        ...
        error: function( jqXHR )
        {
            strictEqual( jqXHR.responseText, "{ \"code\": 40, \"message\": \"Bad Request\" }",
                "Error body properly set" );
            ...
            jQuery.map( [
                ...
                function( options )
                {
                    return jQuery.Deferred( function( defer )
                    {
                        ...
                        {
                            var args = arguments;
                            strictEqual( completeArgs[options.url].length, args.length,
                                "same number of arguments (" + options.type + ")" );
    ...
    asyncTest( "jQuery.post() - data", 3, function()
    {
        jQuery.when(
            ...
            jQuery.ajax( {
                ...
                success: function( data )
                {
                    strictEqual( data, "test%5Blength%5D=7&test%5Bfoo%5D=bar",
                        "Check if a sub-object with a length param is serialized correctly" );
                }
            } )
        ).always( function()
            {
