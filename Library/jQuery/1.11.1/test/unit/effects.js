...
// Supports #7397
    ...
    test( "animate block as inline width/height", function()
    {
        ...
        if (jQuery.support.inlineBlockNeedsLayout || expected === "inline-block") {
            ...
            jQuery( "#foo" ).css( {display: "inline", width: "", height: ""} ).animate( {width: 42, height: 42}, 100,
                function()
                {
                    equal( jQuery( this ).css( "display" ),
                        jQuery.support.inlineBlockNeedsLayout ? "inline" : "inline-block",
                        "inline-block was set on non-floated inline element when animating width/height" );
    ...
    test( "animate native inline width/height", function()
    {
        ...
        if (jQuery.support.inlineBlockNeedsLayout || expected === "inline-block") {
            jQuery( "#foo" ).css( {display: "", width: "", height: ""} )
                ...
                {
                    equal( jQuery( this ).css( "display" ), "inline-block",
                        "inline-block was set on non-floated inline element when animating width/height" );
                    equal( this.offsetWidth, 42, "width was animated" );
                    equal( this.offsetHeight, 42, "height was animated" );
                } );
        ...
        jQuery( "<div>" ).appendTo( "#qunit-fixture" ).css( {
            ...
            complete: function()
            {
                equal( jQuery( this ).css( "display" ), "block",
                    "inline-block was not set on block element when animating width/height" );
        ...
        }, function( tn, t )
        {
            test( fn + " to " + tn, function()
            {
                ...
                jQuery.when( elem ).done( function( elem )
                {
                    ...
                    if (t_w === "hide" || t_w === "show") {
                        ok( f_w === "" ? elem.style.width === f_w : elem.style.width.indexOf( f_w ) === 0,
                            "Width must be reset to " + f_w + ": " + elem.style.width );
                    }

                    if (t_h === "hide" || t_h === "show") {
                        ok( f_h === "" ? elem.style.height === f_h : elem.style.height.indexOf( f_h ) === 0,
                            "Height must be reset to " + f_h + ": " + elem.style.height );
                    }
                    ...
                    if (t_o.constructor === Number) {
                        ...
                        ok( jQuery.css( elem, "opacity" ) !== "" || cur_o === t_o,
                            "Opacity should be explicitly set to " + t_o + ", is instead: " + cur_o );
                    }
                    ...
                    if (t_w.constructor === Number) {
                        ...
                        ok( elem.style.width !== "" || cur_w === t_w,
                            "Width should be explicitly set to " + t_w + ", is instead: " + cur_w );
                    }

                    if (t_h.constructor === Number) {
                        equal( elem.style.height, t_h + "px",
                            "Final height should be " + t_h + ": " + elem.style.height );

                        cur_h = jQuery.css( elem, "height" );

                        ok( elem.style.height !== "" || cur_h === t_h,
                            "Height should be explicitly set to " + t_h + ", is instead: " + cur_h );
                    }
    ...
    test( "Effects chaining", function()
    {
        var remaining = 16,
            ...
            getProps = function( el, hiddenOverflow )
            {
                ...
                {
                    obj[prop] = prop === "overflow" && hiddenOverflow ? "hidden" : el.style[prop] || jQuery.css( el,
                        prop );
                } );
                return obj;
            };
        ...
        setup( ".toggle().toggle(easing) - out", "#easetoggleout div", shrinkwrap ).toggle( "fast" ).toggle( "fast",
            "linear", assert );
        ...
        setup( ".slideUp().slideDown(easing)", "#easeslideup div", shrinkwrap ).slideUp( "fast" ).slideDown( "fast",
            "linear", assert );
        setup( ".slideToggle().slideToggle() - in", "#slidetogglein div",
            shrinkwrap ).slideToggle( "fast" ).slideToggle( "fast", assert );
        setup( ".slideToggle().slideToggle() - out", "#slidetoggleout div",
            shrinkwrap ).slideToggle( "fast" ).slideToggle( "fast", assert );
        setup( ".fadeToggle().fadeToggle() - in", "#fadetogglein div" ).fadeToggle( "fast" ).fadeToggle( "fast",
            assert );
        setup( ".fadeToggle().fadeToggle() - out", "#fadetoggleout div" ).fadeToggle( "fast" ).fadeToggle( "fast",
            assert );
        setup( ".fadeTo(0.5).fadeTo(1.0, easing)", "#fadeto div" ).fadeTo( "fast", 0.5 ).fadeTo( "fast", 1.0, "linear",
            assert );
        this.clock.tick( 400 );
    } );
    ...
    test( "interrupt toggle", function()
    {
        ...
        jQuery.each( {slideToggle: "height", fadeToggle: "opacity", toggle: "width"}, function( method, prop )
        {
            ...
            setTimeout( function()
            {
                ...
                {
                    notEqual( jQuery( this ).css( prop ), jQuery.data( this, "startVal" ),
                        ".stop() before completion of hiding ." + method + "() - #" + this.id );
                } );
                ...
                $methodElems[method]( shortDuration, function()
                {
                    ...
                    {
                        $elem.stop();
                        notEqual( $elem.css( prop ), startVal,
                            ".stop() before completion of showing ." + method + "() - #" + id );

                        // Restore
                        $elem[method]( shortDuration, function()
                        {
                            equal( $elem.css( prop ), startVal,
                                "original value restored by ." + method + "() - #" + id );
                            finish();
                        } );
                    }, shortDuration );
                } );
...
// Start 1.8 Animation tests
    ...
    test( "animate should set display for disconnected nodes", function()
    {
        ...
        strictEqual( $divTest.show()[0].style.display, "block",
            "set display with show() for element with parentNode = document fragment" );
        strictEqual( $divEmpty.show()[0].style.display, "block",
            "set display with show() for element with parentNode = null" );
        strictEqual( $divNone.show()[0].style.display, "block",
    ...
    test( "hide called on element within hidden parent should set display to none (#10045)", 3, function()
    {
        ...
        jQuery.when(
            ...
            {
                strictEqual( elems.get( 0 ).style.display, "none",
                    "hide() called on element within hidden parent should set display to none" );
                strictEqual( elems.get( 1 ).style.display, "none",
                    "hide( 0 ) called on element within hidden parent should set display to none" );
                strictEqual( elems.get( 2 ).style.display, "none",
                    "hide( 1 ) called on element within hidden parent should set display to none" );
    ...
    test( "hide, fadeOut and slideUp called on element width height and width = 0 should set display to none", 5,
        function()
        {
            var foo = jQuery( "#foo" ),
                i = 0,
            ...
            jQuery.when(
                ...
                {
                    strictEqual( elems.get( 0 ).style.display, "none",
                        "hide() called on element width height and width = 0 should set display to none" );
                    strictEqual( elems.get( 1 ).style.display, "none",
                        "hide( jQuery.noop ) called on element width height and width = 0 should set display to none" );
                    strictEqual( elems.get( 2 ).style.display, "none",
                        "hide( 1 ) called on element width height and width = 0 should set display to none" );
                    strictEqual( elems.get( 3 ).style.display, "none",
                        "fadeOut() called on element width height and width = 0 should set display to none" );
                    strictEqual( elems.get( 4 ).style.display, "none",
                        "slideUp() called on element width height and width = 0 should set display to none" );
