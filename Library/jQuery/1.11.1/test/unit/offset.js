(function()
{
    ...
    testIframe( "offset/absolute", "absolute", function( $, iframe )
    {
        ...
        jQuery.each( tests, function()
        {
            ...
            equal( jQuery( this["id"], doc ).offset().left, this["left"],
        ...
        jQuery.each( tests, function()
        {
            equal( jQuery( this["id"], doc ).position().top, this["top"],
                "jQuery('" + this["id"] + "').position().top" );
            equal( jQuery( this["id"], doc ).position().left, this["left"],
    ...
    testIframe( "offset/absolute", "absolute", function( $ )
    {
        ...
        jQuery.each( tests, function()
        {
            ...
            equal( $( this["id"] ).offset().top, this["top"],
                "jQuery('" + this["id"] + "').offset({ top: " + this["top"] + " })" );
            equal( $( this["id"] ).offset().left, this["left"],
                "jQuery('" + this["id"] + "').offset({ left: " + this["left"] + " })" );
            ...
            equal( $( this["id"] ).offset().top, this["top"] + 1,
                "jQuery('" + this["id"] + "').offset({ top: " + (this["top"] + 1) + " })" );
            equal( $( this["id"] ).offset().left, this["left"] + 1,
                "jQuery('" + this["id"] + "').offset({ left: " + (this["left"] + 1) + " })" );
            ...
            $( this["id"] ).offset( {
                "top": this["top"], "left": this["left"], "using": function( props )
                {
                    $( this ).css( {
                        "top": props.top + 1,
                        "left": props.left + 1
            ...
            equal( $( this["id"] ).offset().top, this["top"] + 1,
                "jQuery('" + this["id"] + "').offset({ top: " + (this["top"] + 1) + ", using: fn })" );
            equal( $( this["id"] ).offset().left, this["left"] + 1,
                "jQuery('" + this["id"] + "').offset({ left: " + (this["left"] + 1) + ", using: fn })" );
        } );
    } );
        ...
        jQuery.each( tests, function()
        {
            if (!window.supportsScroll) {
                ...
                if (window.supportsFixedPosition) {
                    equal( $( this["id"] ).offset().top, this["offsetTop"],
                        "jQuery('" + this["id"] + "').offset().top" );
                    equal( $( this["id"] ).position().top, this["positionTop"],
                        "jQuery('" + this["id"] + "').position().top" );
                    equal( $( this["id"] ).offset().left, this["offsetLeft"],
                        "jQuery('" + this["id"] + "').offset().left" );
                    equal( $( this["id"] ).position().left, this["positionLeft"],
                        "jQuery('" + this["id"] + "').position().left" );
                } else {
    ...
    testIframe( "offset/scroll", "scroll", function( $, win )
    {
        ...
        equal( $( "#scroll-1" ).scrollLeft( undefined ).scrollLeft(), 5,
    ...
    test( "chaining", function()
    {
        ...
        equal( jQuery( "#absolute-1" ).offset( coords ).selector, "#absolute-1",
            "offset(coords) returns jQuery object" );
        equal( jQuery( "#non-existent" ).offset( coords ).selector, "#non-existent",
            "offset(coords) with empty jQuery set returns jQuery object" );
        equal( jQuery( "#absolute-1" ).offset( undefined ).selector, "#absolute-1",
            "offset(undefined) returns jQuery object (#5571)" );
    } );
    ...
    test( "offsetParent", function()
    {
        ...
        equal( div[0], document.getElementById( "qunit-fixture" ),
            "The #qunit-fixture is the offsetParent of #nothiddendivchild." );
        ...
        equal( div.offsetParent()[0], document.documentElement,
            "Absolutely positioned div returns html as offset parent, see #12139" );
