...
class ParserTest
{
    ...
    function ObjBuffer()
    {
        ...
        if (!empty( $obj_buffer )) {
            ...
            echo '<textarea id="object_comparison">'.htmlspecialchars( $obj_buffer, ENT_COMPAT, 'UTF-8',
                    false ).'</textarea>';
        }
    ...
    static function showError( $errno, $errmsg, $filename, $linenum, $vars )
    {
        ...
        echo '}return false;">Show Backtrace</a>';
    ...
    function Summary()
    {
        ...
        //time
        echo '<fieldset><legend>Time (PHP):</legend>'.self::microtime_diff( phpless_start_time,
                microtime() ).'</fieldset>';
        echo '<fieldset><legend>Time (Request)</legend>'.self::microtime_diff( $_SERVER['REQUEST_TIME'],
                microtime() ).'</fieldset>';
