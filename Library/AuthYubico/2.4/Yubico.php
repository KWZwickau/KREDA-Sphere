...
class Auth_Yubico
{
    ...
    function getParameters( $parameters )
    {
        ...
        foreach ($parameters as $param) {
            if (!preg_match( "/".$param."=([0-9]+)/", $this->_response, $out )) {
                return trigger_error( 'Could not parse parameter '.$param.' from response' );
            }
    ...
    function verify(
        ...
        if (!$ret) {
            return trigger_error( 'Could not parse Yubikey OTP' );
        }
        $params = array(
            'id'    => $this->_id,
            'otp'   => $ret['otp'],
        ...
        return trigger_error( 'NO_VALID_ANSWER' );
    }
