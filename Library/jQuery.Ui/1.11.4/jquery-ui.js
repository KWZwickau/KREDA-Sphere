    ...
    $.ui.plugin.add( "draggable", "stack", {
        start: function( event, ui, instance )
        {
            var min,
                ...
                {
                    return (parseInt( $( a ).css( "zIndex" ), 10 ) || 0) - (parseInt( $( b ).css( "zIndex" ),
                            10 ) || 0);
                } );
    ...
    $.ui.intersect = (function()
    {
        ...
        return function( draggable, droppable, toleranceMode, event )
        {
            ...
            switch (toleranceMode) {
                ...
                case "pointer":
                    return isOverAxis( event.pageY, t, droppable.proportions().height ) && isOverAxis( event.pageX, l,
    ...
    var sortable = $.widget( "ui.sortable", $.ui.mouse, {
        ...
        _mouseDrag: function( event )
        {
            ...
            for (i = this.items.length - 1; i >= 0; i--) {
                ...
                if (itemElement !== this.currentItem[0] &&
                    this.placeholder[intersection === 1 ? "next" : "prev"]()[0] !== itemElement && !$.contains( this.placeholder[0],
                        itemElement ) &&
        ...
        _intersectsWithPointer: function( item )
        {
            ...
            var isOverElementHeight = (this.options.axis === "x") || this._isOverAxis( this.positionAbs.top + this.offset.click.top,
                ...
                isOverElementWidth = (this.options.axis === "y") || this._isOverAxis( this.positionAbs.left + this.offset.click.left,
                ...
                queries = [
                    [
                        $.isFunction( this.options.items ) ? this.options.items.call( this.element[0], event,
                            {item: this.currentItem} ) : $( this.options.items, this.element ), this
                    ]
                ],
                connectWith = this._connectWith();
                    ...
                    update: function( container, p )
                    {
                        ...
                        if (!p.height()) {
                            p.height( that.currentItem.innerHeight() - parseInt( that.currentItem.css( "paddingTop" ) || 0,
                                10 ) - parseInt( that.currentItem.css( "paddingBottom" ) || 0, 10 ) );
                        }
                        if (!p.width()) {
                            p.width( that.currentItem.innerWidth() - parseInt( that.currentItem.css( "paddingLeft" ) || 0,
                                10 ) - parseInt( that.currentItem.css( "paddingRight" ) || 0, 10 ) );
                        }
                    }
                };
            }
        ...
        _contactContainers: function( event )
        {
            ...
            if (this.containers.length === 1) {
                ...
                itemWithLeastDistance ? this._rearrange( event, itemWithLeastDistance, null,
        ...
        _getParentOffset: function()
        {
            ...
            // 2. The actual offset parent is a child of the scroll parent, and the scroll parent isn't the document, which means that
            //    the scroll is included in the initial calculation of the offset of the parent, and never recalculated upon drag
            if (this.cssPosition === "absolute" && this.scrollParent[0] !== this.document[0] && $.contains( this.scrollParent[0],
                    this.offsetParent[0] )) {
                po.left += this.scrollParent.scrollLeft();
                po.top += this.scrollParent.scrollTop();
            }
            ...
            if (!(/^(document|window|parent)$/).test( o.containment )) {
                ...
                this.containment = [
                    co.left + (parseInt( $( ce ).css( "borderLeftWidth" ),
                        10 ) || 0) + (parseInt( $( ce ).css( "paddingLeft" ), 10 ) || 0) - this.margins.left,
                    co.top + (parseInt( $( ce ).css( "borderTopWidth" ),
                        10 ) || 0) + (parseInt( $( ce ).css( "paddingTop" ), 10 ) || 0) - this.margins.top,
                    co.left + (over ? Math.max( ce.scrollWidth,
                        ce.offsetWidth ) : ce.offsetWidth) - (parseInt( $( ce ).css( "borderLeftWidth" ),
                        10 ) || 0) - (parseInt( $( ce ).css( "paddingRight" ),
                        10 ) || 0) - this.helperProportions.width - this.margins.left,
                    co.top + (over ? Math.max( ce.scrollHeight,
        ...
        _convertPositionTo: function( d, pos )
        {
            ...
            var mod = d === "absolute" ? 1 : -1,
                scroll = this.cssPosition === "absolute" && !(this.scrollParent[0] !== this.document[0] && $.contains( this.scrollParent[0],
    ...
    $.extend( Datepicker.prototype, {
        ...
        _doKeyDown: function( event )
        {
            ...
            if ($.datepicker._datepickerShowing) {
                ...
                if (event.keyCode === 36 && event.ctrlKey) { // display the date picker on ctrl+home
        ...
        _generateHTML: function( inst )
        {
            ...
            (this._isInRange( inst,
                gotoDate ) ? "<button type='button' class='ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all' data-handler='today' data-event='click'" +
